[CmdletBinding()]
param(
  [string]$ExtensionDir = "$env:USERPROFILE\.antigravity\extensions\zk-agi.zk-proxy-pro-9.9.338",
  [string]$ManifestPath = ""
)
...
$required = @(
  ...
  "vendor\外接api\core\zk_router.js"
)
...
if ((Test-Path -LiteralPath $injector) -and
    -not ((Get-Content -Raw -LiteralPath $injector).Contains("<ZK_IDE_CONTEXT_V1>"))) {
