...
- **更新描述**：
  1. 官方账号登录态下本身下发完整模型目录，无需本地拦截合并 GetUserSettings/GetUserStatus 响应，`_isModelUnlockEnabled` 默认关闭并走流式直透，减少缓冲解析开销。
  2. 移除 `autoModelUnlock` 自动轮询及冗余死代码。
  3. 保留 `/origin/model_unlock` 手动接口，特殊权限受限账号仍可通过 POST 恢复全量模型目录注入。
  4. 澄清 `v9.9.519` 期间的 `Failed to send` 真实原因是安装覆盖冲掉了模型改写，而非模型解锁缺失。

---

### 🚀 v9.9.527 (2026-09-04)
- **修复类型**：Bug 修复 (流式空闲超时精准限制与延时)
- **问题描述**：长时间运行的终端任务或大型/思考型模型的长任务请求可能被误杀。
- **修复方案**：
  1. 流式空闲超时判定范围收紧，仅对 SSE (`text/event-stream`) 生效。
  2. 超时时间由 30s 延长至 120s，在防止 Generating 假死的同时充分保障超长思考与大吞吐任务不被提前阻断。

---

### 🚀 v9.9.526 (2026-09-04)
- **修复类型**：Bug 修复 (多轮对话上下文误伤修复) + 垃圾代码清理
- **更新描述**：修复多轮会话IDE上下文注入误伤bug，清理无用死代码
