<#
.SYNOPSIS
  zk-proxy-pro · 卸载善后 · 系统级残留归零 (Windows · 不依赖扩展存活)
.DESCRIPTION
  即便扩展已被 force-remove(deactivate 没跑), 本脚本也能把机器还到「官方直连」零态:
    ① 跨所有 IDE settings.json 清除 zk 锚点/LS 外置重定向键 (写前自动 .bak 备份)
    ② 还原/删除 ~/.codeium/_dao_ls_port.txt (被 zk 覆盖前的官方原值)
    ③ 删除 ~/.codeium/zk-certs/ 目录
...
.EXAMPLE
  ...
  powershell -ExecutionPolicy Bypass -File zk-reset.ps1 -IdeRoot 'D:\Devin'
#>
[CmdletBinding()]
...
Write-Host "zk-proxy-pro · 卸载归零 (Windows)$(if($DryRun){' · DRY-RUN'})" -ForegroundColor Cyan
...
$KEYS = @(
  ...
  'zk.origin._backup_apiServerUrl',
  'zk.origin._backup_inferenceApiServerUrl'
)
...
foreach ($sp in $settingsPaths) {
  ...
  if ($raw -ne $orig) {
    if (-not $DryRun) {
      $bakDir = Join-Path (Split-Path $sp) ".zk-settings-backups"
...
Write-Host "[3/7] zk-certs/ 目录删除"
$certDir = Join-Path $codeium "zk-certs"
if (Test-Path $certDir) {
  if (-not $DryRun) { Remove-Item $certDir -Recurse -Force }
  Did "删除 $certDir"
} else { Step "zk-certs/ 不存在" }
...
Write-Host ""
Write-Host "保留不动: ~/.codeium/zk-byok (主公 key) · ~/.codeium/zk (Cascade 记忆)" -ForegroundColor DarkGray
