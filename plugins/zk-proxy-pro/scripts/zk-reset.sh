#!/usr/bin/env bash
# zk-proxy-pro · 卸载善后 · 系统级残留归零 (macOS / Linux · 不依赖扩展存活)
#   即便扩展已被 force-remove(deactivate 没跑), 本脚本也能把机器还到「官方直连」零态:
#     ① 跨所有 IDE settings.json 清除 zk 锚点/LS 外置重定向键 (写前 .bak 备份)
...
#        bash zk-reset.sh --dry    只报告不改动
#   额外: DAO_IDE_ROOT=/path/to/ide bash zk-reset.sh   (IDE 未装在常见位置时显式指定含 resources/app 的根)
set -u
DRY=0
[ "${1:-}" = "--dry" ] && DRY=1
CHANGED=0
...
echo "zk-proxy-pro · 卸载归零 ($(uname -s))$([ "$DRY" = 1 ] && echo ' · DRY-RUN')"
...
for ide in devin Windsurf Code VSCodium; do
  ...
  if ! cmp -s "$SP" "$TMP"; then
    if [ "$DRY" = "0" ]; then
      BAK="$(dirname "$SP")/.zk-settings-backups"
      mkdir -p "$BAK"
...
echo "[3/6] zk-certs/ 目录删除"
if [ -d "$CODEIUM/zk-certs" ]; then
  [ "$DRY" = "0" ] && rm -rf "$CODEIUM/zk-certs"
  ok "删除 $CODEIUM/zk-certs"
else
  step "zk-certs/ 不存在"
fi
