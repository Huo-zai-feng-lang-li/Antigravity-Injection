...
1. 启动 Devin Desktop + 单一 `zk-proxy-pro`（非归一 zk-one），登录 Pro 账号，确认 `mode=passthrough`。
2. 用 Cascade 选免费档（GLM-5.2 / Kimi K2.7 / SWE-1.6）发一ZK算术题刷新鉴权并探活：
