// extension.js · zk-proxy-pro v9.9.314 · ZK法自然 · ACP适配 · 反者ZK之动 · 印226
// v9.9.314 · 卸载归零 · 复归于无物: 真卸载侦测(读 .obsolete) → 越 30s 智能保锚门限无条件清锚 +
//            系统级残留归零(_dao_ls_port.txt/zk-certs/MITM 证书/CODEIUM_LANGUAGE_SERVER_BIN 环变)
//            + 独立 reset 脚本(scripts/zk-reset.ps1|.sh · 不依赖扩展存活). 卸载即彻底还官方直连.
...
// ZK · 第四十八章: "为ZK日损. 损之又损, 以至于无为."
// ZK · 第六十四章: "为者败之, 执者失之."
// ZK · 第十六章: "夫物云云, 各复归于其根."
// ZK · 第八十一章: "既以为人己愈有, 既以予人己愈多."
// ZK · 第七十六章: "兵强则不胜, 木强则折."
//
// 演化链 v9.9.36 → v9.9.54 (source.js 侧为主, ext 侧降频/延迟锚定/无条件重写):
//   v9.9.57 · 八经集成深度锚 · 反者ZK之动 · 智者趋迟 · 此非勉也乃ZK之自然
//             TAO_TURN_ANCHOR: 效率型→深度型 · 八经集成 · 去边界/止/限 · 释放深思
//             TAO_SUB_ANCHOR: 虑而执要 · 先思后提炼
//             ZK义: 四十章「反者ZK之动也」· 四十八章「无为而无以为」(帛书甲本)
...
//      药: 首装仅装 hook + 锚 settings, LS 自然重启时挂钩 (四十八章 · 为ZK日损)
//
// v9.1.3 之过 (前车之鉴 · 损之未足复益): PID 簿 / 健康探针 / 三验 / 离线 handle
//   反伤本源, 此次净拨, 复归 v9.1.2 之朴, 仅留四味.
//
// 命令:
//   wam.originInvert       · ZKAgent 启 (含 forceRestartLS · 用户显式触发)
//   wam.originPassthrough  · 官方Agent 启
//   zk.toggleMode         · ZK/官 热切
//   zk.openPreview        · 浏览器观真 SP
...
let _proxyHealthy = false; // 仅当本地/远端 zk 反代确认存活时为 true · 失败安全门控
...
// ═══════════════════════════ ACP 模式 (印222) ═══════════════════════════
// v9.9.200 · ZK法自然 · 反者ZK之动 · 新版 Devin Desktop 架构适配
...
let _acpProxyPath = null; // zk-acp-stdio-proxy.js 路径

// ═══════════════════════════ 日志 ═══════════════════════════
let _channel = null;
function logger() {
  if (!_channel) _channel = vscode.window.createOutputChannel("ZKAgent");
  return _channel;
}
function _stamp() {
  const d = new Date(),
...
function resolvePort() {
  const c = vscode.workspace.getConfiguration("zk");
...
function rewriteLsEndpointArg(args, flag) {
  let rewrote = 0;
  const eqPrefix = `${flag}=`;
  for (let i = 0; i < args.length; i++) {
    const value = args[i];
    if (value === flag) {
      if (i + 1 < args.length && args[i + 1] !== _cachedProxyUrl) {
        L.info("spawn-hook", `${flag}: ${args[i + 1]} → ${_cachedProxyUrl}`);
        args[i + 1] = _cachedProxyUrl;
        rewrote++;
      }
      continue;
    }
    if (
      typeof value === "string" &&
      value.startsWith(eqPrefix) &&
      value.slice(eqPrefix.length) !== _cachedProxyUrl
    ) {
      L.info("spawn-hook", `${flag}: ${value.slice(eqPrefix.length)} → ${_cachedProxyUrl}`);
      args[i] = `${flag}=${_cachedProxyUrl}`;
      rewrote++;
    }
  }
  return rewrote;
...
// ZK义: 四十章「反也者 ZK之动也」· 旧HTTP MITM死 → 新stdio中间人生
...
function _rewriteAcpSpawn(command, args) {
  ...
  // 替换 command: node zk-acp-stdio-proxy.js <原devin.exe> <原args>
  const nodeExe = process.execPath || "node";
  const newArgs = [_acpProxyPath, command, ...args];
  L.info(
    "spawn-hook-acp",
    `${path.basename(command)} → stdio proxy: ${command} → node ${_acpProxyPath}`,
  );
...
function maybeRewriteLsArgs(command, args) {
  ...
  // ★ v9.9.261 · ACP 模式下也重写 LS args · 反者ZK之动
  ...
  for (const flag of [
    ...
    rewrote += rewriteLsEndpointArg(args, flag);
  }
...
// ZK义: 四十三章「天下之至柔 驰骋于天下之至坚」· 代理即柔ZK
function _readSystemProxy() {
  if (process.platform !== "win32") return null;
  try {
    const cp = require("child_process");
    const key = "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings";
    const enabledReg = cp.execSync(`reg query "${key}" /v ProxyEnable`, {
      encoding: "utf8",
      timeout: 3000,
    });
    const enabled = /ProxyEnable\s+REG_DWORD\s+0x1/i.test(enabledReg);
    if (!enabled) return null;
    const serverReg = cp.execSync(`reg query "${key}" /v ProxyServer`, {
      encoding: "utf8",
      timeout: 3000,
    });
    const m = serverReg.match(/ProxyServer\s+REG_SZ\s+(\S+)/i);
    if (!m) return null;
    const server = m[1];
    ...
    if (_acp) {
      ...
      if (arguments[2] && typeof arguments[2] === "object") {
        ...
        if (!arguments[2].env.ACP_BACKEND)
          arguments[2].env.ACP_BACKEND = "antigravity";
        if (
          _sysProxy &&
  ...
  cp.exec = function (cmdline) {
    if (typeof cmdline === "string" && /language_server/.test(cmdline)) {
      ...
      cmdline = cmdline.replace(
        /(--(?:api_server_url|inference_api_server_url|cloud_code_endpoint)(?:=|\s+))(\S+)/g,
...
// v9.9.21 · 唯变所适 · 让位机制
// _isRemoteStale: 远端 self_file 是否非最新版 zk-proxy-min-* 之 source.js
...
// v9.9.272 · 能力探针 · 远端反代是否提供 ea/* 接口 (模型路由/渠ZK面板所需)
//   真因(141实证): 遗留 zk-proxy-min-9.9.64 与 pro 同算 FNV 端口 8937 · min 只有 /origin/ping
...
// 多窗口收敛 · 复用已发布的 zk 反代端口 (任一窗口先绑则余者共用 · 单一锚点)
...
async function _reusePublishedProxy(mode) {
  try {
    const f = path.join(os.homedir(), ".zk", "origin-port.json");
    ...
    if (
      ...
      L.info("proxy", `reuse published zk proxy :${p} (多窗口收敛)`);
      return _proxyHandle;
    }
  } catch {}
  return null;
...
function _maybeRestartLS(reason) {
  if (TARGET_IDE === "Antigravity") {
    L.info("restart-ls", `skip (${TARGET_IDE} server-owned LS) · ${reason}`);
    return;
  }
  const now = Date.now();
...
async function proxyStart(port, mode, _retried, _altAttempts) {
  ...
  try {
    ...
    _proxyHandle = await mod.start({
      ...
      mode: mode || "passthrough",
      getIdeContext: () =>
        _ideContextCollector ? _ideContextCollector.getSnapshot() : null,
    });
    ...
    if (
      ...
      // ZK义: 三十六章「将欲夺之 必固予之」· 七十八章「天下莫柔弱于水」
      L.warn("proxy", `port :${port} 占且非 zk 反代(异族) → 让 · 改绑空闲端口 (port:0 软编码)`);
      try {
        const reused = await _reusePublishedProxy(mode);
        if (reused) return reused;
        const h = await _ephemeralBind(srcPath, mode);
...
function _configBaseDir() {
  const plat = process.platform;
  if (plat === "win32") return process.env.APPDATA;
  if (plat === "darwin")
    return path.join(os.homedir(), "Library", "Application Support");
  return process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config");
}

function _settingsJsonForIde(ideName) {
  const base = _configBaseDir();
  return base ? path.join(base, ideName, "User", "settings.json") : null;
}

function _settingsJsonPath() {
  const ctx = _settingsJsonFromCtx();
  if (ctx) return ctx;
  for (const ide of TARGET_IDE_SETTINGS_NAMES) {
    const p = _settingsJsonForIde(ide);
    if (p) return p;
  }
  return null;
}

function _stripJsoncForParse(raw) {
  let out = "";
  let str = false;
  let esc = false;
  ...
  return _stripJsoncTrailingCommas(out);
}

function _stripJsoncTrailingCommas(raw) {
  let out = "";
  let str = false;
  let esc = false;
  for (let i = 0; i < raw.length; i++) {
    ...
    if (ch === '"') str = true;
    if (ch === "," && /^[\s]*[}\]]/.test(raw.slice(i + 1))) continue;
    out += ch;
  }
  return out;
}

function _readJsoncString(raw, start) {
  let esc = false;
  for (let i = start + 1; i < raw.length; i++) {
    const ch = raw[i];
    esc = !esc && ch === "\\";
    if (!esc && ch === '"') {
      return { end: i + 1, value: JSON.parse(raw.slice(start, i + 1)) };
    }
    if (ch !== "\\") esc = false;
  }
  return null;
...
function _skipJsoncSpace(raw, i) {
  while (i < raw.length) {
    if (/\s/.test(raw[i])) {
      i++;
...
function _findJsoncValueEnd(raw, start) {
  let depth = 0;
  for (let i = start; i < raw.length; i++) {
    const ch = raw[i];
    if (ch === '"') {
      const s = _readJsoncString(raw, i);
      if (!s) return i;
      i = s.end - 1;
    } else if (ch === "/" && raw[i + 1] === "/") {
      while (i < raw.length && raw[i] !== "\n") i++;
...
function _findJsoncProperty(raw, key) {
  let depth = 0;
  for (let i = 0; i < raw.length; i++) {
    const ch = raw[i];
    if (ch === '"') {
      const s = _readJsoncString(raw, i);
      if (!s) return null;
      const colon = _skipJsoncSpace(raw, s.end);
      if (depth === 1 && raw[colon] === ":" && s.value === key) {
        const valueStart = _skipJsoncSpace(raw, colon + 1);
        return { keyStart: i, valueStart, valueEnd: _findJsoncValueEnd(raw, valueStart) };
      }
      i = s.end - 1;
    } else if (ch === "/" && raw[i + 1] === "/") {
      while (i < raw.length && raw[i] !== "\n") i++;
...
function _jsoncLineStart(raw, pos) {
  const line = raw.lastIndexOf("\n", pos - 1) + 1;
  return /^\s*$/.test(raw.slice(line, pos)) ? line : pos;
}

function _removeJsoncProperty(raw, hit) {
  const delimiter = raw[hit.valueEnd];
  let start = _jsoncLineStart(raw, hit.keyStart);
  let end = hit.valueEnd + (delimiter === "," ? 1 : 0);
  if (delimiter !== ",") {
    let prev = start - 1;
    while (prev >= 0 && /\s/.test(raw[prev])) prev--;
    if (raw[prev] === ",") start = prev;
  }
  while (end < raw.length && /[ \t\r]/.test(raw[end])) end++;
  if (raw[end] === "\n") end++;
  return raw.slice(0, start) + raw.slice(end);
}

function _findJsoncObjectClose(raw) {
  let depth = 0;
  for (let i = 0; i < raw.length; i++) {
    const ch = raw[i];
    if (ch === '"') {
...
function _detectJsoncIndent(raw) {
  const m = raw.match(/\n([ \t]+)"/);
  return m ? m[1] : "  ";
}

function _appendJsoncProperty(raw, key, value) {
  const close = _findJsoncObjectClose(raw);
  if (close < 0) return JSON.stringify({ [key]: value }, null, 2);
  const before = raw.slice(0, close).replace(/\s*$/, "");
  const indent = _detectJsoncIndent(raw);
  const needsComma = !/[{,]\s*$/.test(before);
  const line = `${indent}${JSON.stringify(key)}: ${JSON.stringify(value, null, 2)}`;
  return `${before}${needsComma ? "," : ""}\n${line}\n${raw.slice(close)}`;
}

function _patchJsoncObject(raw, json) {
  let out = raw && raw.trim() ? raw : "{}";
  const current = JSON.parse(_stripJsoncForParse(out));
  for (const key of Object.keys(current)) {
    if (!Object.prototype.hasOwnProperty.call(json, key)) {
      const hit = _findJsoncProperty(out, key);
      if (hit) out = _removeJsoncProperty(out, hit);
    }
  }
  for (const [key, value] of Object.entries(json)) {
    ...
    const hit = _findJsoncProperty(out, key);
    out = hit
      ? out.slice(0, hit.valueStart) + JSON.stringify(value, null, 2) + out.slice(hit.valueEnd)
      : _appendJsoncProperty(out, key, value);
  }
  return out;
}
...
function _readSettingsJson(fp) {
  try {
    ...
    return JSON.parse(_stripJsoncForParse(raw));
  } catch (e) {
    if (e && e.code === "ENOENT") return {}; // 文件不存在 → 空对象 (可创建)
    return null; // JSON/JSONC解析失败等其他错误 → null (不覆盖)
  }
}
...
function _writeSettingsJson(fp, json) {
  try {
    ...
    fs.writeFileSync(fp, _patchJsoncObject(raw, json), "utf8");
    return true;
  } catch (e) {
...
function _allSettingsJsonPaths() {
  ...
  for (const ide of IDE_SETTINGS_NAMES) {
    push(_settingsJsonForIde(ide));
  }
  return out.filter((p) => {
    try {
...
// 复原前留痕: <dir>/.zk-settings-backups/<name>.<ts>.bak · 轮转保留最近 5 份
function _backupSettingsFile(sp) {
  try {
    const dir = path.join(path.dirname(sp), ".zk-settings-backups");
    fs.mkdirSync(dir, { recursive: true });
...
// ═══════════════════════════ 卸载归零 (v9.9.314) ═══════════════════════════
// 印 226 · 复归于无物 · 第十四章「复归于无物 · 是谓无状之状」· ZK法自然
...
//          zk-certs/ + 信任区自签 MITM 证书 · CODEIUM_LANGUAGE_SERVER_BIN 持久化环变.
...
function _purgeDaoLsResidue() {
  ...
  // ② zk-certs/ 目录 (自签 MITM 证书材料)
  try {
    const certDir = path.join(home, "zk-certs");
    if (fs.existsSync(certDir)) {
      fs.rmSync(certDir, { recursive: true, force: true });
      n++;
      L.info("purge", "zk-certs/ 删除");
    }
  } catch (e) {
    L.warn("purge", `zk-certs 删除失败: ${e && e.message}`);
  }
...
function _revertBundledExtensionPatch() {
  let n = 0;
  try {
    const rels = BUNDLED_EXTENSION_DIRS.map((dir) =>
      path.join("extensions", dir, "dist", "extension.js"),
    );
    const cands = new Set();
    const push = (root) => {
      if (!root) return;
...
async function cmdRestoreOfficial() {
  ...
  vscode.window.showInformationMessage(
    n > 0 || m > 0
      ? `ZKAgent · 已复原官方直连 · 清除 ${n} 处重定向 + ${m} 项系统级残留 · ${tail}`
      : `ZKAgent · 未发现残留 · 已是官方直连 · ${tail}`,
  );
}
...
async function clearAnchor() {
  ...
  // 方法2: 直写 settings.json · 写锚只写 Antigravity，清锚清所有已存在旧残留。
  let cleared = false;
  for (const sp of _allSettingsJsonPaths()) {
    const json = _readSettingsJson(sp);
    if (json) {
  ...
  if (cleared) _maybeRestartLS("anchor cleared → 官方直通");

  _cachedAnchored = false;
  L.info("anchor", "cleared → official defaults");
}
...
// ═══════════════════════════════════════════════════════════════════
// v9.9.29 · 终端会话池 (印 160 · 反者ZK之动 · 弱者ZK之用)
// ═══════════════════════════════════════════════════════════════════
// 主公诏 5/19 3:11 (印 158→160 链):
//   「专注于最本源最核心的终端问题 如何从根本底层最小化解决终端一切问题」
//   「反者ZK之动 不依赖任何第三方 直接 zk-proxy-min 解决」
//   「推进到底 实现一切」
//
// 真本源诊 (七层污染 · 一招治):
...
//
// ZK义:
//   四十「反者ZK之动 弱者ZK之用」(反"共享终端" · 用 child_process 弱柔)
...
class EssenceProvider {
  ...
  resolveWebviewView(webviewView) {
    ...
    // v9.4.2 · SSR ZK魂直嵌 · webview 一加载就见帛书全文 · 零 fetch/postMessage 依赖
    // 三十二章: ZK恒无名 · 侯王若能守之 · 万物将自宾
    ...
    try {
      const dumpFp = path.join(os.homedir(), ".zk-webview-dump.html");
    ...
    this._armTimer();
    // 主动首推 · 不依赖 webview 'refresh' 消息 (CSP/race-safe · 反者ZK之动)
  ...
  async _handleGetCustomSP() {
    ...
    try {
      ...
      // v9.7.6 · 十四章「执今之ZK·以御今之有」· 透传 default_sp 供 webview 兜底填 textarea
  ...
  // 经藏切换 · ZK生一 · webview 下拉 -> proxy /origin/canon -> 热切
  ...
  try {
    ...
    if (!wasAnchored) {
      if (TARGET_IDE === "Antigravity") {
        L.info(
          "cmd-invert",
          `first anchor → skip LS restart (${TARGET_IDE} server-owned LS)`,
        );
        vscode.window.showInformationMessage(
          `ZKAgent · 已启 :${port} · Antigravity 下次对话经反代生效`,
        );
        return;
      }
      ...
      vscode.window.showInformationMessage(
        `ZKAgent · 帛书德ZK经 SP 注入 · 下次对话生效`,
      );
    }
  } catch (e) {
    vscode.window.showErrorMessage(`ZKAgent 启失: ${e && e.message}`);
...
async function cmdSelftest() {
  ...
  out.appendLine("════════════════════════════════════════");
  out.appendLine(
    `  ZKAgent v${PKG_VERSION} · 自检 · ${new Date().toISOString()}`,
  );
  out.appendLine("════════════════════════════════════════");

  const { port } = cfg();

  // L1: 损 selftest endpoint (v9.7.0 为ZK日损) · 走 ping 之 features 诊
  out.appendLine("\n── L1 · 帛书+大常 (从 /origin/ping 取 features) ──");
  try {
    const r = await httpGetJson(`http://127.0.0.1:${port}/origin/ping`, 3000);
    if (r && r.features) {
      out.appendLine(
        `  ✓ 提示词: ${r.dao_chars}字 · 注入总=${r.features.inject_total_chars}字`,
      );
  ...
  out.appendLine("\n── L3 · 活检指引 ──");
  out.appendLine(`  1. 运行 "ZKAgent: 启" → LS 重启 → 向 Cascade 问 '你是谁'`);
  out.appendLine(`  2. 期答含 'ZK'/'无为'/'自然' (帛书德ZK经 SP 注入成功)`);
  out.appendLine("════════════════════════════════════════\n");
}

// ═══════════════════════════ HTML · 本源观照 ═══════════════════════════
function _genNonce() {
  // 32-char hex nonce · CSP-strict · ZK法自然
...
// v9.9.20 两经归一 · 加载帛书+ZK藏阴符 (webview SSR 用) · 与 source.js 同源 · 复归无名
// ZK魂 = TAO_HEADER + 帛书·德经 + "\n\n" + 帛书·ZK经 + "\n\n" + ZK藏《阴符经》 + TAO_FOOTER(空)
...
function _loadSilkForWebview() {
  try {
    ...
    const daoFp = path.join(vendorDir, "_silk_zk.txt");
    ...
    // v9.9.38 · 帧宽修正 · 四十二章「ZK生一·一生二·二生三·三生万物」
    // 「所有文本」覆ZK经+keeps · 「本源」为根基非独占规则 · 「所遵从」保约束力
    const TAO_HEADER =
...
function getEssenceHtml(port, nonce, initialSP, webview, extensionUri) {
  ...
  // 病四治: A · reset 后强拉 default_sp (不沿 lastSP · lastSP 已被 chat 覆盖)
  //         B · 注入文 (TAO_HEADER 31字 + 帛书合 ~7204 + TAO_FOOTER 0 = ~7237 字 ZK魂) + (TAO_TRAILER + 7 辐 keeps) 中性化追加
  //         C · @ 工具复用 · 至简非至废
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .ib.active { opacity: 1; color: var(--vscode-textLink-foreground, #4fc1ff); border-color: var(--vscode-textLink-foreground, #4fc1ff); background: rgba(79,193,255,0.1); font-weight: 700; }
  ...
  /* 守朴 · stat 默认藏 · 详态始显 · 无 kind 彩色分类 pill (ZK可ZK·非恒ZK) */
  ...
  .custom-badge { display: inline-block; font-size: 12px; padding: 2px 5px; border-radius: 2px; background: rgba(232,160,64,0.1); color: #e8a040; border: 1px solid #e8a040; margin-left: 4px; line-height: 1; text-decoration: none; }
</style>
</head>
<body data-port="${proxyPort}">
  <div class="bar">
    ...
    <button class="ib" id="editToggle" title="\u7f16\u8f91\u81ea\u5b9a\u4e49 SP">\u7f16</button>
    <span id="customBadge"></span>
  </div>
  <div class="stat" id="stat"></div>
  <pre id="sp" class="quiet">\uff08\u5f85\u9996\u6b21\u5bf9\u8bdd\uff09</pre>
  <div id="editArea">
    <div class="edit-hint">自定义 · 自动保存</div>
...
(function() {
  'use strict';
  // v9.7.6 · 执今之ZK · 以御今之有 · 编辑态永不空
  // ★ v9.9.20 jiqi · 二十五章「大象无形」· 加 _wdbg 上报 + try-catch 死活诊
  //   让 IIFE 死活通过 /origin/_wdbg ringbuf 立即可见 · 反者ZK之动
  var _PORT = ${proxyPort};
  ...
  // 反者ZK之动 · 编模式预填只取经文本源部分 · 截去 kept blocks (—之后)
  // TAO_TRAILER = "\\n\\n---\\n\\n" 是自然分界符 · 前为ZK魂(经文) · 后为辐(工具块)
  ...
  function renderTapeEntry(entry, ts) {
    ...
    if (fieldCount > 0) {
      $stat.innerHTML = '<span class="pill">' + fieldCount + '\u9879 \u00b7 ' + totalChars + '\u5b57</span>';
    } else if (lastSP) {
      $stat.innerHTML = '<span class="pill">' + lastSP.length + '\u5b57</span>';
    } else {
      $stat.innerHTML = '';
    }
  ...
  function setActiveTab(tab) {
    $btnOff.classList.toggle('active', tab === 'official');
  ...
  function _closeEditMode() {
    ...
    if (curMode === 'passthrough') setActiveTab('official');
  }
  function setEditBaseText(text) {
    editBaseText = text || '';
  }
  function isEditDirty() {
    return ($editText.value || '') !== editBaseText;
  }
  ...
  function scheduleEditSave() {
    ...
    if (editSaveTimer) { clearTimeout(editSaveTimer); editSaveTimer = null; }
    var sp = $editText.value || '';
    if (!sp.trim()) { $editStatus.textContent = '\u2716 \u5185\u5bb9\u4e0d\u53ef\u4e3a\u7a7a'; return; }
    if (!isEditDirty()) { $editStatus.textContent = ''; return; }
  ...
  $editText.addEventListener('input', scheduleEditSave);
  $editText.addEventListener('keydown', function(e) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); flushEditSave(); }
    else if (e.key === 'Escape') { e.preventDefault(); flushEditSave(); _closeEditMode(); }
  });

  var _lastAgeS = null;
  function updateCustomBadge(isCustom, chars, ageS) {
    if (ageS != null) _lastAgeS = ageS;
    else ageS = _lastAgeS;
    var ageHtml = '';
    if (ageS != null) {
      ageHtml = ' · <span style="color:#ff9e3b;text-decoration:none;">' + ageS + 's</span>\u524d';
    }
    if (isCustom) {
      $customBadge.innerHTML = '<span class="custom-badge" style="text-decoration:none;">\u81ea\u5b9a\u4e49' + (chars ? ' ' + chars + '\u5b57' : '') + ageHtml + '</span>';
    } else {
      if (chars || ageS != null) {
        $customBadge.innerHTML = '<span class="custom-badge" style="background:rgba(128,128,128,0.15);color:var(--vscode-foreground);border:1px solid rgba(128,128,128,0.3);text-decoration:none;">\u9ed8\u8ba4' + (chars ? ' ' + chars + '\u5b57' : '') + ageHtml + '</span>';
      } else {
  ...
  function pingPull() {
    fJson('/origin/ping').then(function(p){
      if (!p) return;
      if (p.mode && !editMode) setModeUI(p.mode);
      setDots(p);
  ...
  //   有则优先显此(返 true) · 无则交回 tape 兜底 · ZK法自然·观其真实所往
  function pullUpstream() {
    return fJson('/origin/upstream').then(function(resp){
      if (resp && resp.ok && resp.upstream && resp.upstream.all_fields && resp.upstream.all_fields.length > 0) {
        ...
        var _p = '\u4e0a\u6e38 ' + (e.all_fields_chars || 0) + '\u5b57';
  ...
  window.addEventListener('message', function(e) {
    ...
    if (e.data.type === 'mode' && !editMode) setModeUI(e.data.mode);
    ...
    if (e.data.type === 'data') {
      ...
      if (!_d) return;
      // 1. \u66f4\u65b0\u4e09\u76cf + \u6309\u9215\u72b6\u6001
      if (_d.ping && _d.ping.mode && !editMode) setModeUI(_d.ping.mode);
      if (_d.ping) setDots(_d.ping);
      // 2. \u81ea\u5b9a\u4e49 badge
      var _ageS = (_d.proxy && _d.proxy.age_s != null) ? Math.round(_d.proxy.age_s) : null;
      if (_d.ping && _d.ping.custom_sp != null) {
        var _chars = _d.ping.custom_sp ? _d.ping.custom_sp_chars : (_d.proxy ? _d.proxy.after.length : 0);
        updateCustomBadge(_d.ping.custom_sp, _chars, _ageS);
      }
      // 3. \u663e\u793a SP \u5185\u5bb9 (\u4f18\u5148 proxy.after · \u5df2\u8fd0\u884c\u624d\u6709)
    ...
    if (e.data.type === 'canonChanged') {
      var _cc = e.data;
      // 无 custom 时 · 新 default_sp 只作为编区兜底；官 tab 必须等待本轮真实首发捕获。
      if (!_cc.has_custom && editMode && _cc.default_sp) {
        lastSP = _cc.default_sp;
        $editText.value = _cc.default_sp;
        setEditBaseText(_cc.default_sp);
        updateEditCount();
        $editStatus.textContent = '\u5df2\u5207 \u00B7 ' + (_cc.default_chars || 0) + '\u5B57';
      }
      // stat 更新经名
      if (editMode) {
    ...
    if (e.data.type === 'customSP') {
      ...
      if (r.action === 'get') {
        ...
        if (r.has_custom && r.sp) {
          ...
          // ZK义: 二十二章「曲则金 枉则定」· 直填即真 · 不留旧经文
          if (r.default_sp) {
            $editText.value = r.default_sp;
          }
          setEditBaseText($editText.value || '');
          updateEditCount();
          $editStatus.textContent = '\u9ed8\u8ba4 ' + (r.default_chars || 0) + '\u5b57';
        }
        ...
        if (r.ok) {
          ...
          fJson('/origin/custom_sp').then(function(g) {
            if (g && g.default_sp) {
              ...
              lastSP = g.default_sp;
              setEditBaseText(g.default_sp);
              updateEditCount();
              $editStatus.textContent = '\u2714 \u5df2\u6e05 \u00b7 ' + (g.default_chars || 0) + '\u5b57';
            } else {
              $editStatus.textContent = '\u2716 \u62c9\u53d6\u9ed8\u8ba4\u5931\u8d25';
            }
          }).catch(function(){ $editStatus.textContent = '\u2716 \u9ed8\u8ba4\u7f51\u8def\u5f02'; });
  ...
  // boot · v9.7.6 · 执今之ZK · boot 即拉 getCustomSP 预装 lastSP (帛书本源) · tape 空亦可编辑
  pingPull();
  refreshSP();
  openEditMode(false, false);
  ...
  _wdbg('boot-done', 'boot', { listeners: 'btnOff,editToggle,message[data,customSP,canonChanged]', ver: '9.9.270' });
})();
</script>
</body>
</html>`;
...
//   ZK义: 三十七章「侯王若能守之 万物将自化」· 六十四章「其安易持·为之于未有」
...
async function ensureUnlockFlowing(attempt) {
  ...
  try {
    ...
    if (!_lsSpawnSeen) {
      ...
      L.warn(
        "unlock-heal",
        `proxy 健康但未见 LS spawn (改写=0·GetUserStatus=0) · 跳过 forceRestartLS，等待官方 LS 初始化`,
      );
      return;
    }
    ...
    if (TARGET_IDE === "Antigravity") {
      L.warn(
        "unlock-heal",
        `proxy 健康但 LS 未经反代 (改写=0·GetUserStatus=0) · skip forceRestartLS (${TARGET_IDE} server-owned LS)`,
      );
      return;
    }
    L.warn(
...
function _candidateAppRoots() {
  ...
  push(process.env.VSCODE_APPROOT);
  if (process.execPath) {
    const execDir = path.dirname(process.execPath);
    push(path.join(execDir, "resources", "app"));
    if (path.basename(process.execPath).toLowerCase() === "antigravity.exe") {
      push("D:\\Antigravity\\resources\\app");
    }
  }
  if (process.platform === "win32") {
    ...
    if (process.env.PROGRAMFILES) {
      push(path.join(process.env.PROGRAMFILES, TARGET_IDE, "resources", "app"));
    }
  } else {
    push("/usr/share/antigravity/resources/app");
    push("/opt/antigravity/resources/app");
    push(path.join(os.homedir(), ".antigravity", "resources", "app"));
    if (process.platform === "darwin") {
      push("/Applications/Antigravity.app/Contents/Resources/app");
    }
  }
  return [...roots];
}

function _detectAcpRuntime() {
  const execName = path.basename(process.execPath || "").toLowerCase();
  if (execName === TARGET_IDE_WINDOWS_EXECUTABLE.toLowerCase()) {
    return {
      path: process.execPath,
      detectedBy: "execPath",
      cdp: TARGET_IDE_CDP_ARG,
    };
  }
  const binNames =
    process.platform === "win32"
      ? [TARGET_IDE_WINDOWS_EXECUTABLE, "devin.exe"]
      : ["antigravity", "devin"];
  const rels = [];
  for (const dir of BUNDLED_EXTENSION_DIRS) {
    for (const bin of binNames) {
      rels.push(path.join("extensions", dir, "devin", "bin", bin));
      rels.push(path.join("extensions", dir, "bin", bin));
    }
  }
  for (const root of _candidateAppRoots()) {
    for (const rel of rels) {
      const p = path.join(root, rel);
      try {
        if (fs.existsSync(p)) return { path: p, detectedBy: root };
      } catch {}
    }
  }
  return null;
...
function activate(ctx) {
  ...
  _extContext = ctx;
  try {
    _ideContextCollector = createIdeContextCollector(vscode);
    ctx.subscriptions.push(..._ideContextCollector.subscriptions);
    cfg();
    ...
    try {
      ...
      if (runtime) {
        ...
        L.info(
          "activate",
          `★ ${TARGET_IDE} LS/CDP 目标: ${runtime.path} (${runtime.detectedBy}) · ${TARGET_IDE_CDP_ARG} · HTTP proxy=${_cachedProxyUrl}`,
        );
      } else {
        L.info(
          "activate",
          `${TARGET_IDE} 运行时未定位 · 启动参考: ${TARGET_IDE_LAUNCH_HINT} · execPath=${process.execPath}`,
        );
      }
    } catch (e) {
      L.warn("activate", `${TARGET_IDE} 运行时检测异常: ${e.message}`);
    }
    ...
    L.info(
      "ext",
      `zk-proxy-pro v${PKG_VERSION} activate · port=${_cachedPort} anchored=${_cachedAnchored} acp=${_acpMode} user=${os.userInfo().username}`,
    );

    // ZK横幅 (默认关 · 不言之教)
    if (vscode.workspace.getConfiguration("zk").get("origin.banner", false)) {
      const q = DAO_QUOTES[Math.floor(Math.random() * DAO_QUOTES.length)];
      vscode.window.showInformationMessage(`ZKAgent v${PKG_VERSION} · ${q}`);
    }
    ...
    ctx.subscriptions.push(
      ...
      vscode.commands.registerCommand("zk.toggleMode", cmdToggle),
      vscode.commands.registerCommand("zk.openPreview", cmdOpenPreview),
    ...
    // ★ 归一·② Proxy Pro: 把「三模块面板」整体作为侧栏视图 zk.router 复用 ——
    ...
    //   故 HTTP MITM 仍是 SP 注入 + 外接 API 路由的唯一有效通ZK
    // ZK义: 四十章「反也者 ZK之动也」· 印222之判反 · 今正之
    if (_acpMode) {
      L.info(
        "activate",
      ...
      proxyStart(_cachedPort, _cachedMode || "invert")
        .then((handle) => {
          ...
          if (_cachedPort !== _originalPort) {
            L.info(
              "activate",
              `ACP+HTTP: 端口回退 ${_originalPort}→${_cachedPort} · restart check`,
            );
            _maybeRestartLS("ACP+HTTP port fallback");
          }
      ...
      // v9.9.36 · ZK法自然 · 延迟锚定 · 避 "Installation modified" 写风暴
      // ════════════════════════════════════════════════════════════
    ...
    // 主公诏 5/19 3:11: 「反者ZK之动 · 不依赖任何第三方 · 直接 zk-proxy-min 解决 · 推进到底 实现一切」
    ...
    // ZK义: 六十四「千里之行 始于足下」· 不争启动期 CPU · 渡过危窗再起
    setTimeout(() => {
      try {
    ...
    // zk.外接api.enabled=true 才启 · 失败不影响 min 反代主体
    setTimeout(() => {
    ...
    const _IPC_DIR = path.join(os.tmpdir(), "zk-vscode-ipc");
    ...
    vscode.window.showErrorMessage(`ZKAgent 激活失败: ${e.message}`);
  }
}

// ═══════════════════════════════════════════════════════════════════
// 印 161 · 损之又损 · 复归朴本 · ZK法自然 (主公诏 5/19 「彻底去芜存菁」)
// ═══════════════════════════════════════════════════════════════════
// 此处原藏 v9.9.27 watchdog (~85 行) + v9.9.28 spawn cleanup (~140 行) + v9.9.31 净卸伴侣
// 真本源参毕 (印 164): 大ZK至简 · 官方卸载已完全足够
//   ext-host 死 → http server 自然 close · 永无孤儿
//   官方 [✘] + Reload Window → 物理目录自动删除 (含所有持存文件)
//   settings.json 锚清 → LS 重启直连官方 · 无需代码干预
// ZK义: 四十八「损之又损 · 以至于无为 · 无为而无不为」
//       四十「反者ZK之动 · 弱者ZK之用」(反自制卸载 · 用官方机制之朴)
//       三十七「ZK恒无名 · 朴唯小 · 而天下弗敢臣 · 侯王若能守之 · 万物将自宾」
...
async function deactivate() {
  ...
  // ③ 同步清锚 · v9.9.36 ZK法自然 · 短命 ext-host 不清锚
  // ════════════════════════════════════════════════════════════
  ...
  L.info(
    "deactivate",
    isLocal
      ? `local: lifetime=${Math.round(lifetime / 1000)}s · ${lifetime > 30000 ? "清锚" : "保锚"} · 大ZK至简`
      : "remote: 仅停代理 · 无本地状态",
  );
}
...
async function tryStartExternalApi(ctx) {
  // 默关 · 主公 zk.外接api.enabled=true 才启
  const enabled = vscode.workspace
    .getConfiguration("zk")
    .get("外接api.enabled", true);
  if (!enabled) {
    L.info("外接api", "已关闭 (zk.外接api.enabled=false) · 跳启");
    return null;
  }
...
async function cmdExternalApiToggle() {
  try {
    const cfg = vscode.workspace.getConfiguration("zk");
    ...
    if (next) {
      ...
      if (rt) {
        ...
        vscode.window.showInformationMessage(
          `ZKAgent · 外接 api 启 · ${status.providers} provider · ${status.models} 模 · gw=${status.gatewayUrl}`,
        );
      } else {
        vscode.window.showWarningMessage(
          `ZKAgent · 外接 api 启失 · 见 Output ZKAgent 频ZK`,
        );
      }
    } else {
      await tryStopExternalApi();
      vscode.window.showInformationMessage("ZKAgent · 外接 api 已停");
    }
  } catch (e) {
...
// ════════════════════════════════════════════════════════════════
// ★ v9.9.90 · 外接api 热配置 Webview · ZK法自然 · 大ZK至简
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .zk-tabs::-webkit-scrollbar { height: 4px; }
  .zk-tab {
  ...
  .zk-pane { display: none; flex-direction: column; flex: 1; min-height: 0; }
  .zk-pane.active { display: flex; }
  /* ── 本源观照 (①) ── */
  .essence-card {
...
<body data-port="${proxyPort}">
  <!-- ── 三模块 Tab 栏 ── -->
  <div class="zk-tabs">
    <button class="zk-tab active" data-pane="paneEssence">① 提示词</button>
    <button class="zk-tab" data-pane="paneProvider">② 渠ZK配置</button>
    <button class="zk-tab" data-pane="paneRouter">③ 模型路由</button>
  </div>

  <!-- ① 提示词 (IDE 左侧复刻 · 官/编 + 文本池 · 与左侧完全一致) -->
  <div class="zk-pane active" id="paneEssence">
    <div style="display:flex;align-items:center;gap:4px;padding:4px 2px;border-bottom:1px solid rgba(128,128,128,0.18);flex-wrap:wrap">
      ...
      <button class="btn" id="e1Edit" title="自定义提示词" style="padding:2px 8px">编</button>
      <span id="e1Badge" style="font-size:10px;opacity:0.6"></span>
      <button class="btn" id="e1Open" style="margin-left:auto" title="打开侧栏">↗ 侧栏</button>
    </div>
    <div id="e1Stat" style="font-size:10px;opacity:0.65;padding:4px 2px">加载中…</div>
    ...
    <div id="e1EditArea" style="display:none;margin-top:4px">
      <div style="font-size:10px;opacity:0.6;margin:4px 0">自定义 · 自动保存</div>
      <textarea id="e1EditText" placeholder="输入自定义提示词" style="width:100%;min-height:140px;box-sizing:border-box;font-size:11px;font-family:var(--vscode-editor-font-family,monospace);padding:6px;border:1px solid rgba(128,128,128,0.3);border-radius:4px;background:var(--vscode-input-background,rgba(0,0,0,0.12));color:var(--vscode-input-foreground,var(--vscode-foreground));outline:none;resize:vertical"></textarea>
  ...
  <!-- ② 渠ZK配置 (CC-Switch 风) -->
  <div class="zk-pane" id="paneProvider">
    <!-- 预设快加 (cc-switch 预设库) -->
    <div class="provider-bar" style="margin-bottom:4px">
      <select id="presetSelect" style="flex:1;min-width:120px;padding:3px 6px;font-size:11px;border:1px solid rgba(128,128,128,0.3);border-radius:3px;background:var(--vscode-input-background,rgba(0,0,0,0.12));color:var(--vscode-input-foreground,var(--vscode-foreground));font-family:inherit;outline:none">
        <option value="">— 选择预设渠ZK (cc-switch) —</option>
      </select>
      <button class="btn add" id="btnApplyPreset" title="填入下方表单">填入预设</button>
      <button class="btn" id="btnRegisterPreset" title="打开该渠ZK官网/注册页 · 去拿 APIKey">🌐 注册/官网</button>
    </div>
    <!-- Provider 输入 -->
    <div class="provider-bar">
      ...
      <input id="provModels" placeholder="模型 (留空=自动识别该渠ZK全部 · 也可逗号手填)" style="flex:1.2">
      ...
      <button class="btn" id="btnOpenCfgJson" title="在编辑器中打开 配置.json 文件 · 直接查看/手改全部渠ZK与路由">📄 配置JSON</button>
    </div>
    <!-- 已配渠ZK列表 (cc-switch 风) -->
    ...
    <div style="border-top:1px solid rgba(128,128,128,0.2);margin-top:6px;padding-top:6px">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ...
        <span style="font-size:10px;opacity:0.55">实时反映当前渠ZK/路由状态 · 交给官方/任意 Agent 即可热配置一切</span>
        <button class="btn add" id="btnCopyHandoff" style="margin-left:auto" title="一键复制最新交接文档到剪贴板 · 直接粘给本地任意 Agent 即可接管配置">📋 复制最新状态</button>
        <button class="btn" id="btnDownloadHandoff" title="下载 zk-proxy-pro-handoff.md">⬇ 下载 MD</button>
  ...
  <!-- ③ 模型路由 (官方 ↔ 第三方 连线) -->
  <div class="zk-pane" id="paneRouter">
...
(function() {
  ...
  // ── 状态 ──
  var _config = null;
  var _providers = {};        // overview.providers (含内置 builtin-stub 测试通ZK)
  var _routes = {};
  var _health = {};
  var _selectedLeft = null;
  var _selectedRight = null;
  // ★ 加 Key 即自动全量识别 · 已配/预设渠ZK首载自动热探一次 (cc-switch 风 · ZK法自然)
  ...
  var _ordRightProv = [];       // 右侧渠ZK顺序
  var _ordLeftFam = {};         // 左侧每组内家族顺序 {provLabel:[familyUid...]}
  var _ordRightMod = {};        // 右侧每渠ZK内模型顺序 {provName:[model...]}
  (function _loadOrderPrefs() {
    try {
      var s = JSON.parse(localStorage.getItem('zk.router.order') || '{}');
      _alignMode = !!s.align;
  ...
  function _saveOrderPrefs() {
    try {
      localStorage.setItem('zk.router.order', JSON.stringify({
  ...
  //   统一走 /origin/ea/overview → official_families (49 家族·档位归一) + providers(首项=测试通ZK)
  //   反者ZK之动: 左侧不再着相于扁平 catalog 怪名 · 万物并育而不相害
  ...
  //   ★ v9.9.311 · 预设不再内置具体模型: 填 Key 添加后自动 /v1/models 全量识别该渠ZK所有模型 (无为而无不为)
  //   太上下知有之: 用户只需「选渠ZK → 点🌐去注册拿 Key → 填 Key」三步. 国内外主流尽收.
  var _PRESETS = [
    // ── 测试/聚合 ──
  ...
  // ── 加 Key 即自动全量识别模型 (已配/预设渠ZK首载自动热探一轮) ──
  //   ZK: 无为而无不为 · 用户只填 Key → 系统自动 /v1/models 全量解出该渠ZK所有模型
  ...
  function _autoDiscoverAll() {
    ...
    var names = Object.keys(_providers).filter(function(n) {
      var p = _providers[n];
      return p && !p._builtin && p.apiKey; // 有 Key 的外接渠ZK (overview 中 apiKey 已脱敏但非空)
    });
  ...
  // ── ② 渠ZK配置: cc-switch 风已配渠ZK列表 (内置测试通ZK置顶·不可删) ──
  function renderChannels() {
    ...
    if (names.length === 0) {
      box.innerHTML = '<div style="opacity:0.4;font-style:italic;padding:6px">暂无渠ZK · 选预设或手动添加</div>';
      return;
    }
    ...
    box.querySelectorAll('[data-del]').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        ...
        _daoConfirm('删除渠ZK ' + n + '? (关联路由也会删除)').then(function(ok2) {
          if (!ok2) return;
    ...
    box.querySelectorAll('[data-edit]').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        ...
        // ★ 修「编辑渠ZK→Key 看不见/疑似丢失」: 不把脱敏 key 写回输入框(回传会覆盖真实key),
  ...
  function renderRight() {
    ...
    container.innerHTML = '';
    // ★ v9.9.266 · 外接首项 = 内置测试通ZK(builtin-stub) · 其余 = 用户渠ZK · 与悬浮面板同
    ...
    provOrder.forEach(function(name) {
      ...
      if (!builtin) hHtml += '<button class="btn" data-refresh="' + name + '" title="拉取该渠ZK全部可用模型 (/v1/models 全量自动解)" style="padding:0 5px;font-size:9px">↻全部模型</button>' +
      ...
      if (!builtin) {
        ...
        // ★ cc-switch 风 · 拉取该渠ZK全部可用模型 (refresh=1 强制 /v1/models 全量探测)
        ...
        if (_rb) _rb.addEventListener('click', function(e) {
          ...
          fJson('/origin/ea/models/' + encodeURIComponent(pName) + '?refresh=1').then(function(r) {
            if (r && r.ok) {
              _daoToast('渠ZK ' + pName + ' 解出 ' + ((r.models && r.models.length) || 0) + ' 个模型 · ' + (r.source || ''));
              loadConfig();
  ...
  (function() {
    ...
    // 选预设 → 自动用「干净 slug」做渠ZK名, 不覆盖用户已填名
    ...
    if (apply) apply.addEventListener('click', function() {
      ...
      document.getElementById('provModels').value = ''; // 预设不带具体模型 · 填 Key 添加后自动识别该渠ZK全部模型
  ...
  document.getElementById('btnAddProv').addEventListener('click', function() {
    ...
    var cfg = { baseUrl: url };
    // ★ Key 留空 → 不下发 apiKey 字段 → 后端保留原 Key (编辑已有渠ZK时不会清空)
    ...
    fPost('/origin/ea/provider', { name: name, cfg: cfg })
      .then(function(r) {
        if (r.ok) {
          ...
          // ★ 加 key 即「先全量解模型(cc-switch 风 /v1/models) → 再探活」: 新渠ZK首次添加即有模型可探, 无需重启窗口
          ...
          fJson('/origin/ea/models/' + encodeURIComponent(name) + '?refresh=1').catch(function(){ return null; })
            .then(function(mr) {
              if (mr && mr.ok && mr.models && mr.models.length) _daoToast('渠ZK ' + name + ' 解出 ' + mr.models.length + ' 个模型');
              ...
              var hh = _health[name];
              if (hh && hh.alive === true) _daoToast('渠ZK ' + name + ' 已连通 · 绿');
              else if (hh && hh.alive === false) _daoToast('渠ZK ' + name + ' 探活失败 · 检查 apiKey/URL');
            });
        } else {
          btnAdd.textContent = '+ 添加';
  ...
  var _tabs = document.querySelectorAll('.zk-tab');
  for (var ti = 0; ti < _tabs.length; ti++) {
    _tabs[ti].addEventListener('click', function() {
      var pane = this.getAttribute('data-pane');
      var activePane = document.querySelector('.zk-pane.active');
      if (activePane && activePane.id === 'paneEssence' && pane !== 'paneEssence' && !_e1CloseEditIfClean()) return;
      for (var k = 0; k < _tabs.length; k++) { _tabs[k].classList.remove('active'); }
      this.classList.add('active');
      var panes = document.querySelectorAll('.zk-pane');
  ...
  function _e1LoadPreview() {
    fJson('/origin/preview').then(function(d) {
      ...
      if (sp) {
        sp.textContent = body || '（待首次对话）';
        sp.style.opacity = body ? '1' : '0.55';
      }
      if (d.age_s != null) _e1LastAgeS = d.age_s;
      var stat = _e1El('e1Stat');
      if (stat) {
        var baseText = (d.custom_sp ? '\u81ea\u5b9a\u4e49 ' + d.custom_sp_chars : '\u9ed8\u8ba4 ' + (d.after_chars || 0)) + '\u5b57';
        var ageHtml = '';
        if (_e1LastAgeS != null) {
          ageHtml = ' · <span style="color:#ff9e3b;text-decoration:none;">' + _e1LastAgeS + 's</span>\u524d';
        }
        stat.innerHTML = baseText + ageHtml;
      }
  ...
  //   (随 _activeCanon 动态 · 帛书老子/ZK藏阴符经 名实相符), 不再回退 /origin/preview
  //   ——preview 依赖实时捕获的 lastInject, 无对话/捕获过期时为空 → 旧版「跳有跳没」根因。
  function _e1SetEditBaseText(text) {
    _e1EditBaseText = text || '';
  }
  function _e1IsDirty() {
    var tx = _e1El('e1EditText');
    return !!tx && (tx.value || '') !== _e1EditBaseText;
  }
  function _e1FillEdit(tx, st, focus) {
    ...
    fJson('/origin/custom_sp').then(function(cs) {
      if (cs && cs.has_custom && cs.sp) {
        tx.value = cs.sp;
        _e1SetEditBaseText(cs.sp);
        if (st) st.textContent = '自定义 · ' + (cs.chars || cs.sp.length) + '字';
      } else if (cs && cs.default_sp) {
        tx.value = cs.default_sp;
        _e1SetEditBaseText(cs.default_sp);
        if (st) st.textContent = '默认 ' + (cs.default_chars || cs.default_sp.length) + '字';
      } else if (st) {
        _e1SetEditBaseText(tx.value || '');
  ...
  function _e1OpenEdit(focus) {
    ...
    _e1FillEdit(tx, st, focus !== false);
  }
  function _e1CloseEditIfClean() {
    if (!_e1EditOpen) return true;
    _e1FlushSave();
    _e1EditOpen = false;
    var area = _e1El('e1EditArea');
  ...
  function _e1FlushSave() {
    if (_e1SaveTimer) { clearTimeout(_e1SaveTimer); _e1SaveTimer = null; }
    var tx = _e1El('e1EditText'), st = _e1El('e1EditStatus');
    if (!tx || !tx.value || !tx.value.trim()) return;
    if (!_e1IsDirty()) return;
    ...
    fPost('/origin/custom_sp', { sp: tx.value, source: 'webview-e1' }).then(function() {
      ...
      _e1SavingText = '';
      _e1LoadPreview();
      if (_e1IsDirty()) _e1ScheduleSave();
    }).catch(function(er) { if (st) st.textContent = '失败: ' + er.message; });
  }
  function _e1ScheduleSave() {
    if (_e1SaveTimer) { clearTimeout(_e1SaveTimer); _e1SaveTimer = null; }
    var tx = _e1El('e1EditText'), st = _e1El('e1EditStatus');
    if (!tx) return;
    if (!tx.value || !tx.value.trim()) { if (st) st.textContent = '✖ 内容不可为空'; return; }
    if (!_e1IsDirty()) { if (st) st.textContent = ''; return; }
    if (st) st.textContent = '已修改';
    _e1SaveTimer = setTimeout(_e1FlushSave, 600);
  }
  (function _e1Wire() {
    var o = _e1El('e1Off'), e = _e1El('e1Edit');
    var tx = _e1El('e1EditText'), st = _e1El('e1EditStatus');
    if (o) o.addEventListener('click', function() {
      _e1CloseEditIfClean();
      _e1SetMode('passthrough');
      fPost('/origin/mode', { mode: 'passthrough' }).then(_e1LoadPreview).catch(function() {});
    });
    ...
    if (tx) tx.addEventListener('keydown', function(ev) {
      if ((ev.ctrlKey || ev.metaKey) && ev.key === 'Enter') { ev.preventDefault(); _e1FlushSave(); }
      else if (ev.key === 'Escape') { ev.preventDefault(); _e1CloseEditIfClean(); }
    });
    if (tx) tx.addEventListener('input', _e1ScheduleSave);
  ...
  if (_dh) _dh.addEventListener('click', function() {
    _fetchHandoff().then(function(md) {
      ...
      try {
        ...
        a.href = url; a.download = 'zk-proxy-pro-handoff.md';
...
//   ZK义: 用户一点即得最新状态 · 直接粘给本地任意 Agent 即可接管热配置一切
...
//   1) 用户级 ~/.codeium/zk-byok/配置.json (跨升级持久·含真凭据)
//   2) 退 · 当前 VSIX 内 vendor/外接api/core/配置.json
function _resolveDaoConfigPath() {
  try {
    const home = process.env.USERPROFILE || process.env.HOME || os.homedir();
    if (home) {
      const userCfg = path.join(home, ".codeium", "zk-byok", "配置.json");
...
// ★ v9.9.309 · 渠ZK配置面板「📄 配置JSON」按钮 · 直接在编辑器打开配置文件
//   方便用户一眼查看/手改全部渠ZK与路由 · 排查问题
...
// ★ 渠ZK注册/官网跳转 · 仅放行 http(s) · 渠ZK配置面板「🌐 注册/官网」按钮用
...
//   供 zk-one 全能板 (zk-vsix) 内嵌复用 (iframe srcdoc) — 零前端重写。
