"use strict";
/**
 * runtime.js · 外接api 运行时 · ZK法自然
 * ════════════════════════════════════════════════════════════════
 *
 *   《帛书·四十八章》: "为ZK者日损 · 损之又损 · 以至于无为 · 无为而无不为"
 *   《阴符经》: "天生天杀 · ZK之理也"
 *
 *   职能:
 *     1. 初始化 zk_router.js (模型路由核心)
 ...
 *
 *   接口 (对 source.js):
 *     getRouter()  → zk_router 实例 (null if not ready)
...
const ROUTER_PATH = path.join(CORE_DIR, "zk_router.js");
const WIRE_PATH = path.join(CORE_DIR, "cascade_wire.js");

// ── 配置路径查找 ──
//   1. 用户级: ~/.codeium/zk-byok/配置.json (跨 VSIX install 持久)
//   2. 同目录: core/配置.json (VSIX 内自包含)
//   3. 环境变量: ZK_BYOK_CONFIG
function _resolveConfigPath() {
  if (
    process.env.ZK_BYOK_CONFIG &&
    fs.existsSync(process.env.ZK_BYOK_CONFIG)
  ) {
    return path.resolve(process.env.ZK_BYOK_CONFIG);
  }
  const bundledCfg = path.join(CORE_DIR, "配置.json");
  const bundledTpl = path.join(CORE_DIR, "_默认配置.json");
  const home = process.env.USERPROFILE || process.env.HOME || "";
  if (home) {
    const userDir = path.join(home, ".codeium", "zk-byok");
    const userCfg = path.join(userDir, "配置.json");
    if (fs.existsSync(userCfg)) return userCfg;
    // 用户级配置不存在 → 即刻于用户级播种并接管 · 跨 VSIX install 持久
    //   反者ZK之动: 凭据/渠ZK安于本机 ~/.codeium · 绝不入库 · 升级重装不失
    //   播种优先 bundled 配置.json (含预设渠ZK·迁移既有态), 退 _默认配置.json (无凭据模板)
    try {
      ...
      // 即便无模板亦返回用户路径 · zk_router.init 会内嵌兜底生成于此
      return userCfg;
    } catch (_e) {
  ...
  return bundledCfg; // 默认 (zk_router init 会自动生成模板)
}
...
class ExternalApiRuntime {
  constructor(opts = {}) {
    ...
    this._configKey = opts.configKey || "zk.外接api";
    this._vendorPrefix = opts.vendorPrefix || "zk-";
    this._router = null;
    this._wire = null;
    this._running = false;
    this._gatewayUrl = "";
  ...
  async start() {
    ...
    try {
      ...
      this._log.warn("外接api", `zk_router load fail: ${e.message}`);
    }
...
function ensure(opts = {}) {
  if (!_singleton) {
    ...
    // 但我们可以同步做 init (zk_router.init 是同步的)
    try {
      const Router = require(ROUTER_PATH);
...
//   ZK义: 二十八章「知其白守其辱」· 热添加路由后 _ready=true · 但 _singleton._router=null
...
// ════════════════════════════════════════════════════════════════
// ★ v9.9.90 · 热配置 API 透传 · ZK法自然 · 无为而无不为
...
//   也能直接 require zk_router.js 并 init · 让 webview 始终可读写配置
...
// ★ v9.9.90-fix · 命名对齐: zk_router.js 导出 resetHealthCache / probeAllProviders
...
// ★ v9.9.285 · 最近一次探活快照 (非阻塞) · 供 overview 即时展示渠ZK连通+原因
function hotHealthSnapshot() {
  const R = _getRouterModule();
  return R && R.healthSnapshot ? R.healthSnapshot() : {};
}

// ★ v9.9.285 · 渠ZK响应分类器透传 · 供 test-chat 实证渠ZK伪成功/拒绝
...
//   ZK义: 三十九章「得一以宁」· 得 wire 方能构建正确帧格式
function getWire() {
