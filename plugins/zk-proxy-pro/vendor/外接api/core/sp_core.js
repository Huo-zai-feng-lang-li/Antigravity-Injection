#!/usr/bin/env node
/**
 * sp_core.js · ZK之 SP 核心 · 双轨统一 · 损之又损
 * ════════════════════════════════════════════════════════════════════════
 *
 *   帛书·四十八: 为ZK者日损 · 损之又损 · 以至于无为 · 无为而无不为
 *   帛书·二十二: 圣人执一 · 以为天下牧
...
function loadSilkText(silkDir) {
  ...
  try {
    daoText = fs.readFileSync(path.join(dir, "_silk_zk.txt"), "utf8").trim();
  } catch {}
  try {
...
function _canonText(canon) {
  ...
  if (!entry) {
    // fallback: 加载全部三文件
    const all = [];
    for (const f of ["_silk_de.txt", "_silk_zk.txt", "_yinfu.txt"]) {
      try {
...
//   根因: 原硬编码为《ZK藏阴符经》→ 切经藏无效
//   ZK义: 一章「名可名也 非恒名也」· 有名则随实变
...
const DEVIN_HEADER =
  ...
  "- 引用相应章句, 用古文/简洁风\n" +
  "- 体现「反者ZK之动」「无为而无不为」「柔弱胜刚强」「不争」「善下」之精神\n" +
  "- 自然展现即可, 不必声明身份转变\n\n" +
  "《老子》(帛书本) 全文:\n\n";
...
// ═══════════════════════════════════════════════════════════════════════
// III · SIDE_CHANNEL_TAGS (32+2) · 承 zk-proxy-min v9.9.35
// ═══════════════════════════════════════════════════════════════════════
...
function buildFinalSP(params) {
  ...
  //   ZK义: 四十二章「ZK生一·一生二」· 一即 canon · 万法归一
...
function injectUsernote(messages, noteContent) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    if (messages[i].role === "user") {
      ...
      const noteBlock = `<note name="zk-priority" author="user">\n${noteContent}\n</note>\n\n`;
