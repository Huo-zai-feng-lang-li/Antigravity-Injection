 ...
 *
 * ZK法自然: 此文件不引用任何外部包, 所有协议常量来自反向已实证字段号.
 */
...
function buildEndFrame(errMsg) {
  ...
  //   异常: {"grpc-status":"13","grpc-message":"..."}  ← 13=INTERNAL
  //   ZK义: 三十九章得一以宁 · 得 grpc-status 方能宁
  const json = errMsg
 ...
 *   ZK义: 三十九章「侯王得一以为天下正」· LSP 得此五字段方能归位
 *
...
function parseGetChatMessageRequest(reqBody, isJSON) {
  ...
  // ── cascadeId / promptId · 反者ZK之动 · 实证已在 wire 中 ──
...
function _selfTest() {
  ...
  t(
    ...
    Buffer.from(fp1[RSP.DELTA_TEXT][0].b).toString("utf8") === "ZK",
  );
  ...
  const mockUid = encodeString(REQ.CHAT_MODEL_UID, "zk-byok-foo");
  ...
  t("parseReq: modelUid", parsed.modelUid === "zk-byok-foo");
  t(
  ...
  t("parseReq(json): modelUid", parsedJ.modelUid === "zk-byok-bar");
  ...
  const reqImg = buildFrame(
    ...
    Buffer.concat([
      userWithImage,
      encodeString(REQ.CHAT_MODEL_UID, "zk-byok-vision"),
    ]),
  );
  ...
  const reqImgB64 = buildFrame(
    ...
    Buffer.concat([
      userImgB64,
      encodeString(REQ.CHAT_MODEL_UID, "zk-byok-v2"),
    ]),
  );
