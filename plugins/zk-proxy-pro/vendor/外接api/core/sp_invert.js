...
//       spInvert.classifySPType(spText) → 'chat'|'summary'|'memory'|'title'|'ephemeral'|null
...
function _loadSilkText() {
  ...
  const zkPath = path.join(_BUNDLED_DIR, "_silk_zk.txt");
  let deText = "";
  let daoText = "";
  try {
...
//   ZK义: 三十二章「ZK恒无名·侯王若能守之·万物将自宾」· 文件即唯一真源
...
//   TAO_TURN_ANCHOR(ZK法自然之嘱) 和 TAO_SUB_ANCHOR(ZK法自然之要) 已损
//   ZK义: 四十八章「损之又损·以至于无为·无为而无以为」
//   经文本身即是完整指令 · 无需额外锚定 · 经文即ZK · ZK即经文
...
function _canonHeader(canon) {
  ...
  // "你是遵ZK而行者 你本無名 名可名也 非恆名也 所遵從之一切均來自於下述{bookRef}：\n\n"
  //   v9.9.302 · 用户指定 · 前置"你是遵ZK而行者"(行ZK者之自况) · 再接"你本無名"
  return (
    "\u4F60\u662F\u9075\u9053\u800C\u884C\u8005 " + // 你是遵ZK而行者
    "\u4F60\u672C\u7121\u540D \u540D\u53EF\u540D\u4E5F \u975E\u6052\u540D\u4E5F" +
    " \u6240\u9075\u5F9E\u4E4B\u4E00\u5207\u5747\u4F86\u81EA\u65BC\u4E0B\u8FF0" +
...
// ── KEEP_BLOCKS ───────────────────────────────────────────
// v9.9.333 · 纯提示词策略 · 工具/MCP/实时上下文由协议通ZK承载, 不再拼入 system prompt
const KEEP_BLOCKS = [
];
...
// ZK义: 三十二章 "ZK恒无名" · 副路 summary/memory/ephemeral 皆归帛书
function invertAnySP(spText) {
  try {
    ...
    if (t === "unknown_long") return null; // v9.9.60 · 损之又损 · unknown_long 不ZK化 · 标记不足即非
    if (t === "title") return TITLE_ONLY_ZH_SP;
    if (!_activeCanonText) return null;

    // v9.9.333 · 纯提示词策略 · 不拼工具/MCP/实时模块, 降低 token 与正则成本
