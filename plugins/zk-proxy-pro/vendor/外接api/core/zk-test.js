#!/usr/bin/env node
// zk-test.js · ZKAgent Pro 全链路闭环测试入口
// ZK义: 四十八章「为ZK者日损 损之又损 以至于无为 无为而无不为」
//   损去芜杂 · 归于闭环 · 一键全链路验证
//
// 用法:
//   node zk-test.js              # 全链路 (L1 Wire + L2 路由 + L3 对话 + L4 协议 + L5 深度)
//   node zk-test.js --quick      # 快速 (L1 + L2)
//   node zk-test.js --e2e        # 含端到端 (需代理运行中)
//   node zk-test.js --protocol   # 仅协议验证
//   node zk-test.js --help
...
if (optHelp) {
  console.log(`
${header("ZKAgent Pro · 全链路闭环测试")}
${info("用法:")} node zk-test.js [选项]
...
async function runLspSimulator() {
  console.log(header("\n═══════════════════════════════════════════"));
  console.log(header("  ZK Agent Pro · LSP 全链路模拟器"));
  console.log(header("═══════════════════════════════════════════\n"));
...
async function runProtocolCheck() {
  ...
  // 2. zk_router.js 存在且含关键函数
  const routerPath = path.join(__dirname, "zk_router.js");
  if (fs.existsSync(routerPath)) {
    const src = fs.readFileSync(routerPath, "utf8");
    checks.push({
      name: "zk_router.js _streamOaToCascade 三协议",
      pass:
    ...
    checks.push({
      name: "zk_router.js _sseEventType 追踪",
      pass: src.includes("_sseEventType"),
    });
    checks.push({
      name: "zk_router.js _KNOWN_TOOL_NAMES 含LSP别名",
      pass:
        src.includes("Read") &&
        src.includes("Grep") &&
    ...
    checks.push({
      name: "zk_router.js _STUB_MODELS 已清空",
      pass: src.includes("new Set([])") || src.includes("new Set([") === false,
    });
    checks.push({
      name: "zk_router.js isCustomToolCall 修正",
      pass:
        src.includes("_lspToolNames") && src.includes("_lspCapableToolNames"),
    });
  } else {
    checks.push({ name: "zk_router.js 存在", pass: false });
  }
...
async function runNormalizeCheck() {
  ...
  try {
    ...
    console.log(fail(`zk_router.js 加载失败: ${e.message}`));
    totalFail++;
    return;
  }

  // 临时配置: 仅连模型族 MODEL_SWE_1_6 → deepseek (不写任何档位变体 key)
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zk-norm-"));
...
async function runSeededBaseCheck() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zk-seed-"));
  const cfgPath = path.join(tmpDir, "配置.json");
  fs.writeFileSync(
    cfgPath,
    JSON.stringify({
...
async function main() {
  ...
  console.log(header("╔═══════════════════════════════════════════════╗"));
  console.log(header("║  ZK Agent Pro · 全链路闭环测试               ║"));
  console.log(header("║  为ZK者日损 · 损之又损 · 以至于无为           ║"));
  console.log(header("╚═══════════════════════════════════════════════╝"));
