"use strict";
/**
 * zk_router.js · ZK路由 v2.0 · 透明模型替换 · 反者ZK之动
 * ════════════════════════════════════════════════════════════════
 *
 *   《帛书·四十章》: "反也者，ZK之动也；弱也者，ZK之用也"
 *   《阴符经》: "天之至私，用之至公 · 禽之制在炁"
 *
 *   本源架构 v2.0:
 *     小模型 → cascadeRelay(ZK直连器:7861) → Cascade官方云端(账号池)
...
const { injectOpenAiMessages } = require("../../ide-context-injector");

// ★ v9.9.102 → 修法㉑+ · 上游代理agent (自包含 · 无需 https-proxy-agent 依赖) · ZK法自然
//   根因①: 远程机直连境外 provider 被网络阻断 → connect ETIMEDOUT / TLS 握手失败
//   根因②: 旧实现 require('https-proxy-agent') 在打包环境 MODULE_NOT_FOUND → 静默失败 → 代理形同虚设
//   修复: 纯 Node net+tls 实现 HTTP CONNECT 隧ZK Agent · 无外部依赖
//         代理URL来源: 环境变量 HTTPS_PROXY/HTTP_PROXY → Windows 系统代理(WinINET) · 不硬编码任何端口
//   ZK义: 四十章「弱也者 ZK之用也」· 代理即弱用 · 不直连而通天下 · 不假外求(无外部依赖)
...
// 纯 Node 实现的 HTTP CONNECT 隧ZK https Agent · 无外部依赖
 ...
 *   ZK义: 四十一章「大白如辱」· 代理存在但不可见
 */
function _getProxyAgent(isHttps) {
  // 仅对 https 上游启用隧ZK (http/本地上游保持直连原状)
...
//   ZK义: 最上游 · source.js 已改 SP → 此处仅检测一致性 · 不补丁式修改
...
//   ZK义: 十七章「太上 下知有之」· DAO存在但不可见 · 官方功能完整保留
const _ENHANCE_MARKER = "\n\n<!-- ZK-ENHANCE v9.9.101 -->\n\n";

// ★ v9.9.288 · ZK恒无名 · 去官名归一引擎 (模块级·工具描述+参数描述+SP 共用)
...
//   ZK义: 三十二章「ZK恒无名」· 名去则惑除 · 不改工具名/参数键·仅中性化描述文本。
...
// ★ v9.9.99 · 移植 Go EXE 核心模块 · ZK法自然 · 取之尽锱铢 用之如泥沙
...
try {
  ...
  // 模块缺失时不阻塞 · 降级为旧逻辑
  _log(`[zk-router] ⚠️ 核心模块加载失败: ${e.message} · 降级为旧逻辑`);
}

// ★ v9.9.100 · 废除硬编码桩 · ZK法自然 · 无为而无以为
//   ZK义: 四十八章「损之又损 以至于无为」· 硬编码桩是「为」· 配置路由是「无为」
...
//   ZK义: 二十九章「天下神器 非可为也」· GLM/DeepSeek/Qwen即神器
...
function _routeDiag(msg) {
  try {
    ...
    // v9.9.77 · 异步写入 · 反者ZK之动 · appendFileSync 阻塞事件循环致 ext-host UNRESPONSIVE
    fs.appendFile(_routeDiagPath, `[${t}] ${msg}\n`, () => {});
  } catch {}
}
let _substituteEnabled = false; // 全局开关: substitute模式默认关闭(需用户有目标模型权限)
let _familyTierExtend = false; // ★ 同族档位延伸: 连一档是否覆盖全族 · 默认关(可显式 familyTierExtend:true 开) · 关时 slow 等未显式连线之档保持官方原生直通(默认走官方·免费不路由) · 开时全族档位归一其渠ZK
let _wire = null; // cascade_wire.js (lazy load)

// ════════════════════════════════════════════════════════════════
// ★ v9.9.73 · 内建传输层桩 · 零依赖 · 不假外求
//   ZK义: 三十九章「天得一以清 地得一以宁」· 得一则通
//   用途: swe-1-6 专供传输层验证 · 不依赖任何外部API
//   始终可用 · 始终返回固定响应 · 诊断传输链路每一环节
// ════════════════════════════════════════════════════════════════
const _STUB_TEXT = "ZK可ZK也 非恒ZK也 · 传输层得一 · stub响应正常";
...
async function _stubToCascade(res, w, modelUid, messages, tools, isJSON) {
  ...
  //   ZK义: 三十九章「得一」· 得 message_id + timestamp 方能宁
  const _messageId =
    "bot-" +
  ...
  //   ZK义: 三十九章「侯王得一以为天下正」· LSP 得此二字段方能归位
  {
    const metaParts = [];
...
// ★ v9.9.288 · 上游错误可读回传 · 反者ZK之动
//   根因: 上游渠ZK返回 4xx (如 GitHub Models 免费层限输 8000 token → 真实
...
//   ZK义: 四十章「反者ZK之动」· 错误回传即是助 · 不藏其拙。
function _humanUpstreamError(status, provider, bodySnippet) {
  const _p = provider ? `渠ZK「${provider}」` : "上游渠ZK";
  const _tail = bodySnippet ? `\n\n上游原文: ${String(bodySnippet).slice(0, 300)}` : "";
  if (status === 413) {
    return `⚠️ ${_p} 拒绝请求 (HTTP 413 · 请求体过大)。\n\n该渠ZK对单次输入有 token 上限 (如 GitHub Models 免费层约 8000 token)，而当前请求(系统提示 + 工具定义 + 对话历史)已超限。\n\n建议: ① 换用额度更高的渠ZK; ② 精简上下文 / 减少同时启用的工具; ③ 新开对话以缩短历史。${_tail}`;
  }
  if (status === 401 || status === 403) {
    return `⚠️ ${_p} 鉴权失败 (HTTP ${status})。API Key 无效、过期或无该模型权限。请在面板②检查该渠ZK的密钥与权限。${_tail}`;
  }
  if (status === 429) {
    return `⚠️ ${_p} 触发限流 (HTTP 429)。请求过于频繁或额度已耗尽，请稍后重试或更换渠ZK。${_tail}`;
  }
  if (status === 400 || status === 422) {
    return `⚠️ ${_p} 拒绝请求 (HTTP ${status} · 参数不合法)。可能是该模型不支持当前的工具/参数组合。${_tail}`;
  }
  if (status === 404) {
    return `⚠️ ${_p} 返回 404。该模型名在此渠ZK不存在，请在面板③确认路由的目标模型名是否正确。${_tail}`;
  }
  return `⚠️ ${_p} 返回错误 HTTP ${status}。请检查渠ZK配置或稍后重试。${_tail}`;
}
...
async function _errorToCascade(res, w, modelUid, isJSON, errText) {
  try {
    ...
    const _actualModelUid = modelUid || "zk-router";
...
//   根因: 每次热保存(加渠ZK/解模型)都改写配置.json → 触发 fs.watch → init() 全量重载 →
//         _providers 被替换为新对象 · 与正在进行的「解模型→探活」内存改写打架 →
//         解出的 models 不落、探活退化用渠ZK名当模型 → 首次添加必失败、须重启+手点探测。
//   修正: 自写内容与磁盘一致时跳过热重载 · 仅外部手改方重载 (ZK法自然·不扰己流)。
let _lastSelfWriteData = null;

// ★ v9.9.81 · 服务端工具补充 · init() 中填充
//   LSP 不发这些工具但官方后端知ZK → DeepSeek 需要才能调用
let _serverToolDefs = [];
...
//   ZK义: 十七章「太上不知有之」· LSP 不知有代理 · 工具调用自然流转
//
...
// 估算成本 (仅当渠ZK配置了 pricing:{inPer1k,outPer1k} 时给出 · 否则 null)
...
function wire() {
  ...
  //   ZK义: 十六章「致虚极也」· 虚其缓存 · 方能重新得
  const cwPath = path.join(__dirname, "cascade_wire.js");
  if (require.cache[cwPath]) {
    delete require.cache[cwPath];
    _log("[zk-router] wire() · cascade_wire.js cache cleared");
  }
  try {
    _wire = require(cwPath);
  } catch (e) {
    _log(`[zk-router] cascade_wire load fail: ${e.message}`);
  }
  return _wire;
}
  ...
  try {
    ...
    if (!fs.existsSync(configPath)) {
      ...
      if (fs.existsSync(templatePath)) {
        _log("[zk-router] 配置.json 不存在 · 从 _默认配置.json 复制");
        try {
          fs.copyFileSync(templatePath, configPath);
        } catch (ce) {
          _log("[zk-router] 复制模板失败: " + ce.message);
        }
      }
    }
    // 仍然不存在则内嵌生成 (兜底)
    ...
    if (!fs.existsSync(configPath)) {
      _log("[zk-router] 配置.json 不存在 · 内嵌无为模板生成");
      const defaultCfg = {
        _ZK: "外接api · ZK法自然 · 无为模板 · 只路由SWE 1.6 Fast · 其他走官方",
      ...
      try {
        ...
        _log("[zk-router] 默认配置已写入: " + configPath);
      } catch (we) {
        _log("[zk-router] 写入默认配置失败: " + we.message);
      }
    }
    ...
    if (dr.enabled === false) {
      _ready = false;
      _log("[zk-router] daoRoutes.enabled=false · 透明路由已禁用");
    ...
    //   ZK义: 二章「为而弗恃」· 四十八章「损之又损 以至于无为」· 损去多余之桩
    ...
    for (const _pn of Object.keys(_providers)) {
      ...
      if (_pc && _pc.baseUrl) {
        ...
        if (_nb && _nb !== String(_pc.baseUrl).replace(/\/+$/, "")) {
          _log(
            `[zk-router] [载] provider ${_pn}: baseUrl 自愈归一 ${_pc.baseUrl} → ${_nb}`,
          );
          _pc.baseUrl = _nb;
        }
      }
    ...
    if (!_substituteEnabled) {
      _log("[zk-router]   substitute模式: 关闭 (substituteEnabled=false)");
    }

    // ★ 同族档位自动延伸开关 (连一档是否覆盖全族) · 默认关闭(可显式 familyTierExtend:true 开)
    //   关(默认): swe-1-6-slow 等未显式连线之档保持官方原生直通 · 即「默认走官方·免费不路由」
    //     —— 用户旨意: fast 档随首个渠ZK自动路由 · slow 档默认官方 · 仅显式连线方路由第三方
    //   开(可选): 同族任一档位被显式连线 → 全族档位(含 catalog 无独立项的 slow)归一其渠ZK
    //   守常: 仅延伸「含真实非播种路由」之族 · 纯播种桩族(无真路由)仍保官方直通
    //   ZK义: 二十五章「ZK法自然」· 唯变所适 · 默认顺其自然(官方) · 用户欲连方延伸
    ...
    _log("[zk-router]   MCP工具: " + (_allowMcpTools ? "允许" : "仅白名单"));
    ...
    if (_ready) {
      _log("[zk-router] ══════════════════════════════════════════");
      _log(`[zk-router] ZK路由 v2.0 就绪 · routes=${count}`);
      const provCounts = {};
      ...
      for (const [p, n] of Object.entries(provCounts)) {
        ...
        _log(
          `[zk-router]   ${p}: ${n}条 · url=${pCfg.baseUrl || _gatewayUrl} · protocol=${proto}`,
        );
      }
      ...
      for (const [uid, t] of Object.entries(_routes)) {
        ...
        _log(`[zk-router]   ${parts.join(" ")}`);
      }
      _log("[zk-router] ══════════════════════════════════════════");
    } else {
      _log("[zk-router] 无路由配置");
    }

    wire();

    // ★ v9.9.62 · 配置.json 文件监听 · 变化时自动热重载 · ZK法自然
      ...
      try {
        ...
        _cfgWatcher = fs.watch(cfgDir, (eventType, filename) => {
          ...
          _cfgDebounce = setTimeout(() => {
            ...
            //   根除「加渠ZK/解模型 → 触发 watch → init() 重载 → 内存改写被冲掉」的竞态
            try {
            ...
            _log("[zk-router] 配置.json 外部变化检测 · 自动热重载...");
            try {
              ...
              _log("[zk-router] 配置热重载异常: " + e.message);
            }
          }, 500);
        });
        _log("[zk-router] 配置监听已启动 · " + configPath);
      } catch (e) {
        _log("[zk-router] 配置监听启动失败 (不影响运行): " + e.message);
      }
    }

    // ★ v9.9.99 · 初始化 Go 移植模块 · ZK法自然
    ...
    try {
      ...
      _log(
        "[zk-router] ★ Go移植模块初始化完成: budget + adapters + resilience",
      );
    } catch (e) {
      _log(`[zk-router] ⚠️ Go移植模块初始化失败: ${e.message} · 降级为旧逻辑`);
    }
    ...
    _log(`[zk-router] init 失败: ${e.message}`);
    _ready = false;
...
function extractModelUid(rawBody, isJSON) {
  try {
    const w = wire();
    if (!w) {
      _log("[zk-router] extractModelUid: wire()=null");
      return null;
    }
    const parsed = w.parseGetChatMessageRequest(rawBody, !!isJSON);
    if (!parsed) {
      _log(
        "[zk-router] extractModelUid: parsed=null isJSON=" +
          isJSON +
          " bodyLen=" +
    ...
    if (!parsed.modelUid) {
      _log(
        "[zk-router] extractModelUid: modelUid='' isJSON=" +
          isJSON +
          " bodyLen=" +
          (rawBody ? rawBody.length : 0) +
          " keys=" +
...
//   用于"连一档即覆盖全族"的同族匹配 · ZK义: 二十五章「ZK法自然·名异实同」
...
function _normalizeModelUid(uid) {
  ...
  //     (slow 等档 catalog 无独立项·无法单独连线 → 必经此延伸方能命中用户所连渠ZK)
  ...
  if (base && base !== uid) {
    ...
    //     不得吞并 slow/fast/lite 变体之真实聊天流 → 让其落入 3.6 择优真实渠ZK
    const _real = (k) => {
      const r = _routes[k];
  ...
  //   ZK义: 二十八章「朴散为器·大制无割」· 四十八章「损之又损·以至无为而无不为」
  ...
  if (_qFam) {
    const _sibs = Object.keys(_routes)
      ...
      .sort();
    if (_sibs.length) {
      // ★ v9.9.284 · 兄弟档位择优 (VM实证修正): 真实外接渠ZK > builtin-stub/substitute
...
async function route(req, res, rawBody, isJSON, modelUid) {
  ...
  // ★ v9.9.67 · 诊断日志 — 路由入口
  _log(
    `[zk-router] route() entry: modelUid=${modelUid} normalized=${normalized} target=${target ? target.provider + "/" + target.model : "null"} isJSON=${isJSON} bodyLen=${rawBody ? rawBody.length : 0}`,
  );
  _routeDiag(
  ...
  const w = wire();
  if (!w) {
    _log(`[zk-router] [SKIP] cascade_wire 不可用 · ${modelUid}`);
  ...
  try {
    ...
    // ★ v9.9.67 · 诊断: 解析后消息数
    _log(
      `[zk-router] parse ok: msgs=${parsed.messages?.length || 0} tools=${parsed.tools?.length || 0} sysLen=${parsed.system?.length || 0}`,
    );
    _routeDiag(
    ...
    _log(`[zk-router] [SKIP] parse 失败: ${e.message} · ${modelUid}`);
    _routeDiag(
  ...
  //   ZK义: 十六章「万物旁作 吾以观其复也」· 观其所发 · 方知其所缺
  try {
  ...
  try {
    const getter = globalThis.__DAO_GET_IDE_CONTEXT;
    const snapshot = typeof getter === "function" ? getter() : null;
    if (snapshot) injectOpenAiMessages(messages, snapshot);
  } catch (e) {
    _log(`[zk-router] IDE context skip: ${e && e.message}`);
  }

  // ★ v9.9.99 · Token 预算管理 (移植自 Go budget.Apply)
  //   ZK义: 四十四章「知足不辱 知止不殆 可以长久」
  ...
  if (_budget && target && target.provider !== "builtin-stub") {
    try {
      ...
      //   ZK义: 无为而无以为 · 知而不妄动 · 超限时方裁
    } catch (e) {
      _log(`[zk-router] budget.apply 异常: ${e.message}`);
    }
  }

  // ★ v9.9.99 · 协议自动检测 (移植自 Go adapters.detectProtocol)
  //   ZK义: ZK法自然 · 自动识别 · 无需手动指定
  ...
  if (_adapters && target && target.provider !== "builtin-stub") {
    try {
      ...
      _log(`[zk-router] detectProtocol 异常: ${e.message}`);
    }
  }

  // ★ v9.9.92 · 修法⑦ · SP 检测+日志 · 最上游 · 不修改
  //   ZK义: source.js 已在最上游修改 SP → 此处仅检测一致性
  //   若 SP 未ZK化 → 日志警告 (source.js 可能未生效)
  //   若 SP 已ZK化 → 日志确认 (全链路一致)
  //   不做任何修改 · 不做任何补丁 · 只观不造
  if (
    messages.length > 0 &&
    ...
    if (_spInvert) {
      if (_spInvert.isAlreadyInverted(_sysContent)) {
        _routeDiag(
          `[zk-router] [SP已ZK化] ✓ 幂等确认 ${_sysContent.length}B · source.js最上游已生效`,
        );
      } else if (_sysContent.indexOf("<!-- ZK-ENHANCE") >= 0) {
        // ★ v9.9.101 · 太上下知有之 · 增强模式 · 官方SP保留 + DAO增强
        _routeDiag(
          `[zk-router] [SP已增强] ✓ 太上模式 ${_sysContent.length}B · 官方SP保留 + DAO增强 · 工具指令完整`,
        );
      } else if (_spInvert.isLikelyOfficialSP(_sysContent)) {
        _routeDiag(
          `[zk-router] [SP未ZK化] ⚠ 官方SP ${_sysContent.length}B · source.js可能未生效 · 透传`,
        );
        // ★ 不修改 · 最上游原则 · 仅日志
      } else {
        ...
        if (_spType) {
          _routeDiag(
            `[zk-router] [SP类型=${_spType}] ${_sysContent.length}B · 非chat主路 · source.js副路处理`,
          );
        }
      }
    }
  ...
  // ★ v9.9.83 · LSP 原始工具名集合
  //   ZK义: 二十八章「知其白 守其辱」· 知 LSP 所发 · 守 IS_CUSTOM_TOOL 正位
  ...
  if (target.provider === "builtin-stub") {
    ...
    try {
      ...
      _log("[zk-router] [stub✗] " + modelUid + ": " + e.message);
    }
    _stats.errors++;
  ...
  if (!provCfg || provCfg.enabled === false) {
    _log(
      `[zk-router] [SKIP] provider=${target.provider} 不存在或已禁用 · ${modelUid}`,
    );
    ...
    if (target.fallback && target.fallback.provider) {
      ...
      if (fbProvCfg && fbProvCfg.enabled !== false) {
        _log(
          `[zk-router] [FB→] ${modelUid} → ${fbTarget.provider}/${fbTarget.model}`,
        );
        try {
          ...
          _log(`[zk-router] [FB✗] ${modelUid}: ${e.message}`);
        }
      }
    }
    _stats.errors++;
  ...
  try {
    ...
    _log(`[zk-router] [✗] 主路由异常 ${modelUid}: ${e.message}`);
  }
  if (primaryOk) {
    _stats.routed++;
    return true;
  ...
  if (target.fallback && target.fallback.provider) {
    ...
    if (!fbProvCfg || fbProvCfg.enabled === false) {
      _log(
        `[zk-router] [FB✗] fallback provider=${fbTarget.provider} 不存在或已禁用`,
      );
    } else {
      _log(
        `[zk-router] [FB→] ${modelUid} → ${fbTarget.provider}/${fbTarget.model}`,
  ...
  _stats.errors++;
  _log(`[zk-router] [✗] ${modelUid} 所有路由失败 · 不回退官方`);
  _routeDiag(`route() ALL FAIL: ${modelUid} headersSent=${res.headersSent}`);

  // ★ v9.9.288 · 反者ZK之动 · 上游错误可读回传 (不再对话死亡)
  ...
  if (!res.headersSent && callOpts && callOpts._lastErr) {
    ...
    if (_sent) {
      _log(
        `[zk-router] [↩] ${modelUid} 上游错误已可读回传 Cascade (HTTP ${_e.status} · ${_e.provider})`,
      );
      return true;
    }
  }
...
async function _tryRoute({
  ...
  //   ZK义: 三十九章「侯王得一以为天下正」· 得一则正
  if (target.provider === "builtin-stub") {
    _log("[zk-router] [stub→] " + modelUid + " → builtin-stub (传输层验证)");
    _routeDiag(
  ...
  if (isPrimary && provCfg.healthCheck) {
    ...
    if (!alive) {
      _log(
        `[zk-router] ${tag}[SKIP] ${target.provider} 健康检查失败 · ${modelUid} · url=${hcUrl}`,
      );
      return false;
    }
  }

  _log(`[zk-router] ${tag}[→] ${modelUid} → ${target.provider}/${sendModel}`);
  try {
    ...
    while (true) {
      ...
      if (agRes.statusCode !== 200) {
        ...
        _log(
          `[zk-router] ${tag}[✗] HTTP ${agRes.statusCode}: ${errBody.slice(0, 180)}`,
        );
        _routeDiag(
    ...
    if (
      ...
      while (_remainingServerCalls.length > 0 && _retryCount < _MAX_RETRIES) {
        _retryCount++;
        _log(
          `[zk-router] ⚡ 内部重试 #${_retryCount}: ${_remainingServerCalls.length} 个服务端工具`,
        );
        _routeDiag(
        ...
        //   ZK义: 五章「虚而不淈 踵而俞出」· 流不塞则通 · 心跳不息则连接不竭
        //   LSP 客户端约 10 秒无数据则 abort → 重试调用可能耗时 10+ 秒
        //   发送 thinking 帧让 LSP 知ZK连接仍活跃
        ...
        if (retryRes.statusCode !== 200) {
          ...
          _log(
            `[zk-router] ⚡ 内部重试 HTTP ${retryRes.statusCode}: ${errBody.slice(0, 180)}`,
          );
          break;
        }
        ...
        if (_remainingServerCalls.length === 0) {
          _log(
            `[zk-router] ⚡ 内部重试 #${_retryCount} 完成: 无更多服务端工具调用`,
          );
          break;
        }
      ...
      if (_needFinalize) {
        ...
        if (!res.writableEnded) res.end();
        _log(`[zk-router] ⚡ 流最终化: stopReason=${_finalStopReason}`);
      }

      if (_retryCount >= _MAX_RETRIES && _remainingServerCalls.length > 0) {
        _log(
          `[zk-router] ⚡ 内部重试达到上限 ${_MAX_RETRIES}: ${_remainingServerCalls.length} 个未处理`,
        );
      }
    }

    // ★ v9.9.301 · 用量聚合 (按渠ZK/模型) · 供「外接API」面板查看
    try {
      _recordUsage(
        target.provider,
        sendModel,
    ...
    _log(
      `[zk-router] ${tag}[✓] ${modelUid} → ${target.provider}/${sendModel}`,
    );
    _routeDiag(
    ...
    _log(`[zk-router] ${tag}[✗] ${target.provider} 异常: ${e.message}`);
    _routeDiag(
...
function _checkHealth(name, healthUrl) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      _log(
        `[zk-router] _checkHealth URL parse fail: ${healthUrl} · ${e.message}`,
      );
      return resolve(false);
    }
    const mod = u.protocol === "https:" ? https : http;
    // ★ 健康探测需带鉴权: 第三方模型站 /v1/models 多需 Bearer · 无 key 必 401 → 误判 DEAD(红点)
    //   ZK义: 三十九章「得一以宁」· 得 apiKey 之全方能验真
    ...
    const req = mod.request(
      ...
      (res) => {
        ...
        _log(
          `[zk-router] _checkHealth ${name}: ${alive ? "ALIVE" : "DEAD"} (${res.statusCode}) url=${healthUrl}`,
        );
        resolve(alive);
      },
    );
    req.on("error", (e) => {
      ...
      _log(
        `[zk-router] _checkHealth ${name} ERROR: ${e.message} url=${healthUrl}`,
      );
      resolve(false);
    });
    req.on("timeout", () => {
      req.destroy();
      _healthCache[name] = { alive: false, ts: Date.now() };
      _log(`[zk-router] _checkHealth ${name} TIMEOUT url=${healthUrl}`);
      resolve(false);
    });
    req.end();
  });
...
async function _callProvider(
  ...
  return (async () => {
    ...
    if (Array.isArray(tools) && tools.length > 0) {
      ...
      //   ZK义: 官方API不过滤工具 · DeepSeek也不应过滤 · 所有工具透传
      ...
      //   ZK义: ZK恒无名 · 记忆体系本属官方着相 · 模型根本不应知有"记忆"
      //   主剔除在 source.js 最上游(两路同净);此处为外接路径防御补刀
      //   (source.js 旧版/被绕过时仍保第三方渠ZK无记忆工具暴露)。
      ...
      //   修复: 发往渠ZK前中性化描述中的官方产品名 → "you"/"the editor"
      //   (与ZK化 SP「你本無名」一致 · 以"你"称之) · 不改工具名/参数 · 机制不破。
    ...
    //   此处是热重载安全区 · zk_router.js 的修改会被 _eaHotReload 自动生效
    if (
      ...
      if (!_isInverted && !_isEnhanced && _isOfficial) {
        ...
        _routeDiag(
          `[zk-router] [SP已增强] ✓ 太上模式 ${_spText.length}B · 官方SP保留 + DAO增强 · 工具指令完整`,
        );
      }
      // ★ v9.9.288 · ZK恒无名 · SP 去官名 (发渠前最后一步)
      //   根因: 官方SP尾部默认工作目录 "CascadeProjects" 等产品名随增强SP透传给真实渠ZK
      ...
      if (typeof messages[0].content === "string") {
        ...
        if (_spAfter !== _spBefore) {
          ...
          _routeDiag(
            `[ZK恒无名·SP去名] ${_spBefore.length}B → ${_spAfter.length}B (中性化官方产品名)`,
          );
        }
      }
    }
    ...
    //   ZK义: 十四章「执今之ZK·以御今之有」· 观其真实所往 · 不滞 devin 侧经文
    ...
    if (provCfg.baseUrl && provCfg.modelPrefix) {
      ...
      // ── 直连模式: 有 baseUrl 即直连真实渠ZK (model原名直发) ──
      //   ZK义: 名实相符 · 配了 baseUrl 就发到 baseUrl · 不再丢给本地兜底网关
      //   修复: 缺 noProviderPrefix 的真实渠ZK(如 deepseek/github)曾被错丢到
    ...
    //   ZK义: 十六章「致虚极也 守情表也」· 致虚于日志 · 守情于实证
    //   反者ZK之动 · 不知实则不可修 · 知实则修之无碍
    try {
...
function _executeServerTool(name, argsJson) {
  ...
  switch (name) {
    ...
    case "read_notebook": {
      ...
      _log(
        `[zk-router] ⚠️ _executeServerTool ${name}: 不应到达此处 (LSP有执行器) → 返回不可用`,
      );
    ...
    case "check_deploy_status": {
      _log(
        `[zk-router] _executeServerTool ${name}: not available in proxy mode`,
      );
      return JSON.stringify({
    ...
    case "skill": {
      _log(
        `[zk-router] _executeServerTool skill: not available in proxy mode`,
      );
      return JSON.stringify({
    ...
    default: {
      _log(`[zk-router] _executeServerTool unknown: ${name}`);
      return JSON.stringify({
...
async function _unaryOaToCascade(
  ...
  try {
    ...
    _log("[zk-router] _unaryOaToCascade: JSON parse fail: " + e.message);
    return;
  }
  ...
  if (!res.writableEnded) res.end();
  _log(
    `[zk-router] unary ✓ text=${Buffer.byteLength(msg.content || "")}B tools=${(msg.tool_calls || []).length} actualModelUid=${_actualModelUid} outputId=${_outputId}`,
  );
}
...
function _streamOaToCascade(
  ...
  const _flushTools = () => {
    ...
    for (const tc of _dedupedCalls) {
      ...
      if (isProxyOnlyTool) {
        ...
        _log(
          `[zk-router] ⚡ 拦截仅代理工具: ${tc.name} args=${(tc.argumentsJson || "").substring(0, 120)}`,
        );
      } else {
  ...
  //   ZK义: 二十八章「知其白 守其辱 为天下式」· 知各协议之白 · 守其转换之辱
  ...
  return new Promise((resolve, reject) => {
    ...
    agRes.on("end", () => {
      ...
      if (_resilience && stopReason === w.STOP_MAX_TOKENS) {
        ...
        if (shouldContinue) {
          _log(
            "[zk-router] ★ resilience: finishReason=length → 可自动继续 (累积text可重发)",
          );
          _routeDiag(
      ...
      //   ZK义: 三十七章「ZK恒无为」· 流不闭方能续 · 闭则割裂
      if (_serverSideCalls.length > 0) {
        // 有服务端工具 → 不发送 stop_reason/EOS/end → 等待 _tryRoute 重试
        _log(
          `[zk-router] stream ▶ text=${textBytes}B think=${thinkBytes}B tools=${toolCount} stopReason=${stopReason} serverSideIntercepted=${_serverSideCalls.length} → stream held open for retry`,
        );
        _routeDiag(
      ...
      // ★ ZK法自然 · 防「上游干净结束却无 finish_reason → IDE 视为中断」
      //   部分渠ZK/代理在最后一帧不带 finish_reason、或缺 data:[DONE] 即收流;
      //   只要已有产出(text/think/tool) → 按正常结束补 STOP_END · 令本轮完整可继续。
      //   ZK义: 二十五章「周行而不殆」· 行至其终当善成 · 不令半途若断。
      if (
        stopReason === null &&
        ...
        _log(
          "[zk-router] ★ 上游无 finish_reason 但有产出 → 补 STOP_END (防误判中断)",
        );
      }
      ...
      if (!res.writableEnded) res.end();
      _log(
        `[zk-router] stream ✓ text=${textBytes}B think=${thinkBytes}B tools=${toolCount} stopReason=${stopReason} tokens=${_tokenCount.input}+${_tokenCount.output} serverSideIntercepted=0`,
      );
      _routeDiag(
    ...
    agRes.on("error", (e) => {
      ...
      _flushTools();
      // ★ ZK法自然 · 优雅中断恢复 (修「对话对一半就断、无法继续」核心)
      ...
      //   仅当【零产出】(刚发起就断) 才回硬错误。
      //   ZK义: 七十八章「天下莫柔弱於水 而攻堅強者莫之能勝」· 遇断不折 · 顺势成全。
      const _hadOutput =
      ...
      if (_hadOutput) {
        _log(
          `[zk-router] ⚠ 上游流中断(${e.message}) · 已产出 text=${(_textAccum || "").length}B → 优雅封口 STOP_END(本轮可继续)`,
        );
      ...
      if (w.buildEndFrame) {
        try {
          res.write(w.buildEndFrame(`ZK路由上游错误: ${e.message}`));
        } catch {}
      }
...
function resetHealthCache() {
  ...
  _log("[zk-router] 健康缓存已清空");
}

/**
 * 拼出 provider 的模型列表 URL · 防 baseUrl 已含 /vN 时重复拼 /v1 → /v1/v1/models 404
 *   ZK义: 二十二章「曲则全」· baseUrl 多形(含/不含版本段) · 归一方得真路
 */
// ★ cc-switch 风 · 渠ZK地址归一: baseUrl 末若含补全路径则剥离 · 防双重路径
//   ZK义: 二十二章「曲则全」· 多形归一方得真路
//   实证根因: 小米 baseUrl=https://api.xiaomimimo.com/v1/chat/completions
//     + completionPath=/v1/chat/completions → 拼成
//     .../v1/chat/completions/v1/chat/completions → 404 → 渠ZK完全不可用。
...
//   ZK义: 四十八章「为ZK日损」· 候选有序 · 逐一损去不通之路 · 至于得真
//   不着相于单一 URL · 万物并育而不相害 · 任意渠ZK皆可自动解全量模型
...
//   ZK义: 二十七章「善行无辙迹」· 自动回填只取可对话之模 · 不把 tts/asr/embedding 当对话模型路由 (必失败)
const _NON_CHAT_RE =
...
//   ZK义: ZK法自然 · 不着相于配置 · 从云端实证模型能力自识协议 (如 freemodel claude 系 → anthropic)
function _autoDetectProtocolFromModels(provCfg, dataArr) {
  ...
  if (anyAnthropic && !anyOpenAI) {
    ...
    _log(
      `[zk-router] [discover] ${provCfg.baseUrl || "?"} · 模型自识为 anthropic 协议 → completionPath=${provCfg.completionPath}`,
    );
  }
}

// ★ 渠ZK级错误/拒绝模式 · 区别于「内容拒绝」(resilience.matchesRefusal 处理模型内容层)
//   这些是网关/渠ZK层的「伪成功」(HTTP 200 却是拒绝文案) 或鉴权失败文案
//   根因(实证): freemodel 返回 HTTP 200 + "Access Denied...official client only" →
//     仅看状态码必误判为通(绿点) · 须看响应体方知其不通
//   ZK义: 二十一章「名实相符」· 通则言通 · 不通则明言其不通
...
// ★ v9.9.308 · 渠ZK失败之「可读中文提因」· 名实相符 · 让用户一眼知该如何处置
//   实证(火山方舟): key 有效但账号未开通模型 → 404 ModelNotOpen → 原仅显笼统 "HTTP 404"
//   ZK义: 七十一章「知不知 尚矣」· 不止报不通 · 更明言因何不通、当往何处治
const _CHANNEL_HINTS = [
  ...
  },
  {
    ...
    hint: "渠ZK拒绝(HTTP 200 伪成功) · 该中转仅限官方客户端/已授权访问",
  },
  {
  ...
  },
  {
    ...
    hint: "中转/上游暂不可用(常为 503/502) · 多为该中转后端临时故障 · 稍后重试或换渠ZK",
  },
];
...
/**
 * 渠ZK响应分类 · 实证渠ZK是否真通 (不止看 HTTP 码 · 还看响应体伪成功/拒绝文案)
 *   返回 { ok, reason } · ok=false 时 reason 简述不通之因 (供前端展示给用户)
 *   ZK义: 七十一章「知不知 尚矣」· 知其不通方能明言其不通
 */
function classifyChannelResponse(status, text) {
  ...
  for (const p of _CHANNEL_ERR_PATTERNS) {
    if (p.test(raw)) {
      return {
        ok: false,
        reason: (hint ? hint + " · " : "") + `渠ZK拒绝/伪成功 · ${snippet}`,
      };
    }
  }
...
/**
 * 实证探活 · 发一条最小真实 chat 请求 · 端到端验渠ZK是否真通
 *   不止探 /models (仅证 key 有效) · 更探真实推理是否被拒
 *   (如 freemodel: /models=200 却 chat 返回 Access Denied · github: /models=404 却 chat 通)
 *   ZK义: 四十八章「损之又损」· 损去表层探测之伪 · 直取真发之实
 */
function _verifyProviderChat(name, cfg) {
  return new Promise((resolve) => {
    // ★ 探活用模型必须是渠ZK「真实模型」· 绝不退化用渠ZK名当模型
    //   旧法 `|| name`: 无 models 时把渠ZK名(如 deepseek)当模型发 → 上游 400
    //   「The supported API model names are ... but you passed <渠ZK名>」→ 误判探活失败。
 ...
 *  ★ 实证探活 (v9.9.285): 发一条最小真实 chat · 端到端验渠ZK真伪
...
async function probeAllProviders() {
  ...
  for (const [name, cfg] of Object.entries(_providers)) {
    ...
    //   根因: 首次添加渠ZK时 models 尚空 → 直接探活会无模型可用 → 误判失败 · 须重启+手点。
    ...
    _log(
      `[zk-router] probe(chat) ${name}: ${v.alive ? "ALIVE" : "DEAD"}` +
        (v.reason ? ` · ${v.reason}` : "") +
        ` · model=${v.model}`,
    );
...
/** 最近一次探活快照 (非阻塞 · 供 overview 即时展示渠ZK连通+原因 · 不重新出网)
 *   ZK义: 四十七章「不出于户 以知天下」· 缓存即知 · 不扰真流
 */
...
function hotAddProvider(name, cfg) {
  ...
  //   实证: 小米渠ZK用户把完整端点贴进 baseUrl → 拼 completionPath 双重 → 完全不可用 · 此处根治
  if (cfg.baseUrl) {
    // ★ baseUrl 去内部空白 · 根治「http s://…」被切成 "http" 的渠ZK不可用 (URL 不含空格)
    ...
    if (_normBase && _normBase !== String(cfg.baseUrl).replace(/\/+$/, "")) {
      _log(
        `[zk-router] [热] provider ${name}: baseUrl 归一 ${cfg.baseUrl} → ${_normBase} (剥补全后缀·防双重路径)`,
      );
  ...
  //   ZK义: 二十二章「曲则全」· baseUrl 多形(含/不含版本段) · 归一方得真路 (与 _modelsUrlFor 同源)
  ...
  // ★ apiKey 保全 (修「编辑渠ZK→key 消失」根因之一)
  //   ZK义: 三十九章「得一以宁」· 得 apiKey 之全方能宁
  //   两类需保留原 key 的情形:
  //     (1) 传入脱敏 key (含***) — GET overview/providers 返回的是脱敏值
  //     (2) 传入空 key — 编辑既有渠ZK时用户未重输 key (前端清空了输入框)
  ...
  if ((!_incomingKey.trim() || _keyMasked) && existing.apiKey) {
    ...
    _log(
      `[zk-router] [热] provider ${name}: ${_keyMasked ? "脱敏" : "空"}apiKey → 保留原key (${existing.apiKey.length}B)`,
    );
  }
  // 单渠ZK单 key · 清理历史遗留的 apiKeys[]/endpoints[] · 重新保存即净化配置
  delete merged.apiKeys;
  delete merged.endpoints;
  _providers[name] = merged;
  _log(
    `[zk-router] [热] 添加provider: ${name} type=${cfg.type} url=${cfg.baseUrl || "?"}`,
  );

  // ★ v9.9.262 · 首个 provider 自动默认路由 · ZK法自然 · 无为而无不为
  ...
  if (!_hasSweFast && merged.enabled !== false) {
    ...
    if (m) {
      ...
      autoRoute = `${name}/${m}`;
      _log(
        `[zk-router] [热] 首provider自动默认路由: MODEL_SWE_1_6_FAST → ${name}/${m}`,
      );
    }
  }
...
function hotRemoveProvider(name) {
  ...
  if (refs.length > 0) {
    ...
    for (const [uid] of refs) {
      delete _routes[uid];
      _log(`[zk-router] [热] 自动移除路由: ${uid} (provider=${name} 被删)`);
    }
  }
  delete _providers[name];
  delete _healthCache[name];
  _log(
    `[zk-router] [热] 删除provider: ${name} · 关联路由${refs.length}条已移除`,
  );
  _hotSaveConfig();
...
function hotAddRoute(modelUid, routeCfg) {
  ...
  _ready = true; // 有路由即就绪
  _log(
    `[zk-router] [热] 添加路由: ${modelUid} → ${routeCfg.provider}/${routeCfg.model}` +
      (routeCfg.fallback
...
function hotRemoveRoute(modelUid) {
  ...
  _ready = Object.keys(_routes).length > 0;
  _log(`[zk-router] [热] 删除路由: ${modelUid}`);
  _hotSaveConfig();
  return { ok: true };
}
...
function hotSetConfig(newCfg) {
  ...
  try {
    ...
    _ready = Object.keys(_routes).length > 0;
    _log(
      `[zk-router] [热] 批量设置配置: providers=${Object.keys(_providers).length} routes=${Object.keys(_routes).length}`,
    );
    _hotSaveConfig();
    return { ok: true };
  } catch (e) {
...
function _hotSaveConfig() {
  try {
    ...
    //   ZK义: 六十四章「为之于未有 治之于未乱」· 临时文件+rename 原子落盘 · 旧本留痕
    _atomicSaveWithBackup(configPath, data);
  } catch (e) {
    _log(`[zk-router] _hotSaveConfig exception: ${e.message}`);
  }
}
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        _log(`[zk-router] 配置备份失败(忽略): ${be.message}`);
      }
    }
    ...
    fs.writeFile(tmp, data, "utf8", (err) => {
      if (err) {
        _log(`[zk-router] _hotSaveConfig tmp 写入失败: ${err.message}`);
        try {
          fs.unlinkSync(tmp);
        } catch {}
        return;
      ...
      fs.rename(tmp, configPath, (rerr) => {
        if (rerr) {
          _log(`[zk-router] _hotSaveConfig rename 失败: ${rerr.message}`);
          try {
            fs.unlinkSync(tmp);
          } catch {}
        } else {
          _log(
            `[zk-router] _hotSaveConfig ok(原子): ${configPath} ${data.length}B`,
          );
        }
      });
    });
  } catch (e) {
    _log(`[zk-router] _atomicSaveWithBackup exception: ${e.message}`);
  }
}
...
async function hotListProviderModels(providerName, opts) {
  ...
  //   ZK义: 五十七章「我无为也 而民自化」· 用户只输 apiKey · 模型自现全量
...
function hotSetResilienceParam(key, value) {
  ...
  // 注意: 弹性配置是运行时的 · 不持久化
  _log(`[zk-router] [热] resilience.${key} = ${JSON.stringify(value)}`);
  return { ok: true };
}
