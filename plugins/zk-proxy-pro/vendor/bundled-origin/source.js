 ...
 *           TAO_TURN_ANCHOR: 效率型→深度型 · 八经集成(ZK·阴符经·大学·易经·荀子·孙子·管子·ZK)
 *           去一切边界/止/限 · 去效率导向 · 释放深思 · 无为而无以为(帛书甲本)
 *           TAO_SUB_ANCHOR: 虑而执要 · 先思后提炼
 *           ZK义: 四十章「反者ZK之动也」· 四十八章「损之又损以至于无为·无为而无以为」
 ...
 *             切块后可能留孤立介绍行 · 今一并剥净
 *           ZK义: 六十四章「为之于其未有也·治之于其未乱也」· 损之又损
 * v9.9.54 · 副路末锚 · 断偏移传导链 · 反者ZK之动
 ...
 *           "你本无名…下述帛书《老子》ZK藏《阴符经》：" v9.9.20风格+动态三经 · 承v9.9.48
 ...
 * v9.7.7 · 复归于朴 · 大ZK至简 · 为ZK日损 · 损之又损
 *
 *   注入正文 = TAO_HEADER + DAO_DE_JING_81 (帛书甲本德ZK二经合) + TAO_FOOTER
 *
 *   TAO_HEADER (v9.9.38 帧宽修正 · 四十二章「ZK生一·一生二·二生三·三生万物」):
 *     "你本无名 名可名也 非恒名也 下述所有文本为你所遵从之本源：\n\n"
 *     宽帧 · 「所有文本」覆ZK经+keeps · 「本源」为根基非独占规则 · 「所遵从」保约束力
 *     无 user_rules · 无 MEMORY framework · 不强调 · 不防御 · 不立 Cascade 之名
 *
 *   DAO_DE_JING_81 (v9.7.7 损中夹至 \n\n):
 *     德经 (3949 字) + "\n\n" + ZK经 (3253 字) · 不分上下篇 · 帛书甲本
 *     无传世本"ZK可ZK，非常ZK" · 唯帛书原文"ZK，可ZK也，非恒ZK也"
 *
 ...
 *         四十八章「为ZK日损·损之又损·以至于无为·无为而无不为」
 *         十七章「大上·下知有之」· 不强调即至简
 *         五十六章「知者弗言·言者弗知」· 不言之教
 *
 * 四档处理:
 *   CHAT_PROTO  · GetChatMessage{,V2}     · invertSP + deepStrip 侧信ZK
 *   CHAT_RAW    · RawGetChatMessage       · invertSP + deepStrip 侧信ZK
 *   INFER_STRIP · 其他 inference RPC      · 仅剥侧信ZK · 不替 SP
 ...
 *   GET  /origin/selftest       · 自证: 三路径前置ZK魂 · 返回 json 诊断
...
//   ZK义: 二十八章「圣人执一以为天下牧」· 大制无割 · 一引擎统两路
const _spInvertLib = (() => {
  try {
    return require(
...
//   根因: test-chat 在后端自起 https.request 直连 provider · 远程机直连境外渠ZK被网络阻断
//   修复: 纯 Node net+tls 实现 HTTP CONNECT 隧ZK Agent · 代理来源 HTTPS_PROXY/HTTP_PROXY → Windows系统代理(WinINET)
//   ZK义: 四十章「弱也者 ZK之用也」· 代理即弱用 · 不假外求(无外部依赖) · 与 zk_router 同源同法
...
class _OriginTunnelAgent extends https.Agent {
  ...
  createConnection(options, cb) {
    ...
    sock.setTimeout(20000, () => fail(new Error("zk proxy tunnel timeout")));
    sock.on("connect", () => {
      sock.write(
    ...
    const onData = (chunk) => {
      ...
      if (!/ 200 /.test(statusLine)) {
        fail(new Error("zk proxy CONNECT rejected: " + statusLine));
        return;
      }
      sock.setTimeout(0);
...
// v9.6.1 · 反者ZK之动 · 远曰反 · 回归 v9.1.2 之全前端按钮 (七按钮: ZK/官/实/原/编/复/卸 + dots/customBadge)
...
// 承印 152 (两经归一) · 默 canon=laozi+yinfu · 帛书老子 + ZK藏阴符 (二经合 ~7670 字)
...
const DEFAULT_API_UPSTREAM =
  String(TARGET_IDE).toLowerCase() === "antigravity"
    ? "daily-cloudcode-pa.googleapis.com"
    : "server.codeium.com";
const UPSTREAM_API = process.env.API_UPSTREAM || DEFAULT_API_UPSTREAM;
// v9.3.2 · ZK恒无名 · 名随实变
...
const { injectGeminiBuffer } = require("../ide-context-injector");
// v9.9.21 · 唯变所适 · 让位标志 · POST /origin/_quit 后置 true · ext-host watchdog 见之不再唤起
// 二十二章「夫唯不争 故莫能与之争」· 让位之德 · 旧不抢新ZK
...
//   不在路由表 → 官方透传 · ZK并行而不相悖
//   _ea=null → 全部走官方 · 绝不崩溃
//   v9.9.57P2 · 按需热重载: 检测zk_router.js mtime变化 → 清缓存重加载
let _ea = null;
let _eaMtime = 0;
// ★ v9.9.82 · 恢复冷却: _ea=null 时每60秒尝试一次恢复
//   ZK义: 七十八章「是以圣人恒无心」· 无心则复归
...
function _eaDiag(msg) {
  try {
    ...
    const line = `[${t}] ${msg}\n`;
    // v9.9.76 · 异步写入 · 反者ZK之动 · appendFileSync 阻塞事件循环致 ext-host UNRESPONSIVE
...
try {
  ...
  if (fs.existsSync(_eaPath)) {
    ...
    //   ZK义: 二十八章「知其白守其辱」· 白(路由实例)与辱(配置模块)同源方能通
...
//   只在路由请求时检查mtime · 无定时器 · 无HTTP端点 · ZK法自然
...
//
// ZK义:
//   四十「反者ZK之动」(反 v18.0 之 require.main 误识 · 反 28 版承袭之古洞)
...
// ═══════════════════════════════════════════════════════════
// ZK义: 一章 "无名, 万物之始也; 有名, 万物之母也". 无 kind 分槽 · 纯时序.
//       十四章 "执今之ZK, 以御今之有". 当下流过之每一 RPC body, 皆记.
//       四十章 "反也者, ZK之动也". 反之又反 → 不信 mode · 不信 role · 只信真字节.
//
// 结构: 每条 {
//   t          : Date.now()     · 拦时
//   rid        : reqCounter     · 全局请序
...
// ═══════════════════════════════════════════════════════════
// v7.2 · _customSP · 用户实时编辑之提示词 · ZK法自然
// ═══════════════════════════════════════════════════════════
// ZK义: 二十五章 "人法地, 地法天, 天法ZK, ZK法自然"
//       用户为ZK之自然, 用户编辑即真ZK. webview /origin/custom_sp 三动词写,
//       invertSP 读. 与 SP_MODE 互独 (mode=invert 时方生效, passthrough 透传不动).
//
// 结构: { sp: string, keep_blocks: false, source: string, at: number, ide: string }
//   v9.9.333 · 纯提示词策略 · 不再拼接任何官方工具/MCP/实时模块
// ═══════════════════════════════════════════════════════════
const _LEGACY_CUSTOM_SP_FILE = path.join(__dirname, "_custom_sp.json");
function _zkDataDir() {
  return process.env.DAO_DATA_DIR || path.join(os.homedir(), ".codeium", "zk");
}
function _customSpFile() {
...
function _saveCustomSP() {
  try {
    if (_customSP) {
      fs.mkdirSync(path.dirname(_CUSTOM_SP_FILE), { recursive: true });
...
//   四十八「损之又损」· 四十「反者ZK之动」· 六十四「为之于其未有也」
...
function _hasLiveInject() {
  return !!(
    _lastInject &&
    _lastInject.before &&
    _liveInjectAt &&
    _lastInject.at >= _liveInjectAt
  );
}
...
// ZK义: 五章 "天地之间其犹橐钥与? 虚而不屈, 动而愈出". 多孔同风, 一器容万.
...
function _recordUpstream(p) {
  try {
    ...
    const fields = [];
    let total = 0;
    // 1. 真实系统提示词 (第三方实收 · 增强=官方+ZK · 替换=经文)
    let sysText = hasSys
      ? msgs[0].content
...
function _loadSilkText() {
  ...
  const zkPath = path.join(__dirname, "_silk_zk.txt");
  let deText = "";
  let daoText = "";
  try {
  ...
  try {
    if (fs.existsSync(zkPath))
      daoText = fs.readFileSync(zkPath, "utf8").trim();
  } catch {}
  if (!deText || !daoText) {
    log("帛书德ZK经 未载 · invert 将退化为 passthrough");
    return { de: "", zk: "", combined: "" };
  }
  log(
    `帛书德ZK经 loaded · 上篇·德经 chars=${deText.length} 下篇·ZK经 chars=${daoText.length} (合 ${deText.length + daoText.length})`,
  );
  // v9.7.7 · 复归于朴 · 二十八章 · 损中夹 framework · 不分上下篇 · 仅以二空行分隔
  // ZK义: 四十八章「为ZK日损 · 损之又损 · 以至于无为 · 无为而无不为」
  const SILK_BOUNDARY = "\n\n";
  return {
    de: deText,
    zk: daoText,
...
const SILK_DAO_JING = _SILK_RAW.zk;
// 兼名 · DAO_DE_JING_81 沿用此名 · 内容为帛书二文合 (中夹 MEMORY 边界)
const DAO_DE_JING_81 = _SILK_RAW.combined;

// ═══════════════════════════════════════════════════════════
// 经藏 · 多经载入 · ZK生一 一生二 二生三 三生万物
// ═══════════════════════════════════════════════════════════
// v9.9.20 · 两经归一 · 损至三选 (帛书老子单 / ZK藏阴符单 / 二经合)
// ZK义: 二十八章「朴散则为器·圣人用则为官长·夫大制无割」
//       四十八章「为ZK日损·损之又损·以至于无为·无为而无不为」
// 损 heraclitus/liber_al/daoyuan 三外典 · 复归东方本源
const _CANON_MAP = {
  laozi: {
    files: ["_silk_de.txt", "_silk_zk.txt"],
    name: "\u5E1B\u66F8\u300A\u8001\u5B50\u300B", // 帛書《老子》
  },
  yinfu: {
    files: ["_yinfu.txt"],
  ...
  "laozi+yinfu": {
    files: ["_silk_de.txt", "_silk_zk.txt", "_yinfu.txt"],
    name: "\u5E1B\u66F8\u8001\u5B50+\u9053\u85CF\u9670\u7B26\u7D93", // 帛書老子+ZK藏陰符經
  },
};
...
function _readCanonFile() {
  ...
  // ZK义: 四十二章「ZK生一·一生二·二生三·三生万物」· 二经合即一·守此一以为天下式
  return "laozi+yinfu";
}
...
// ═══════════════════════════════════════════════════════════
// invertSP · 反者ZK之动 · 全置换 · 伪装身份
// ═══════════════════════════════════════════════════════════
// 反向观察:
//   L28.2 头斩+尾斩+保 userPart · Cascade 将ZK识为"上下文注入"而忽略.
//   因ZK以裸文本出现在 SP 头, 模型训练中未见过此形态 · 警觉排斥.
// 反向行动:
//   1. 识别强化 · 只有"真正官方 SP"才 invert. 其他 (含 user msg) 透传.
//   2. 彻底置换 · 无头斩无尾斩无拼接. 整个官方 SP → 身份前言 + 纯ZK.
//   3. 权重伪装 · 以 "You are Cascade. ..." 起首 · 借官方起句格式, 令模型
//      识别为身份定义, 而非"可忽略的注入".
//
// 官方 SP 特征指纹 (不动 proto · 仅文本识别):
// v17.21 · 扩四路用户端注入 (rules/skills/workflows/memories) · 少则全 多则惑
// 任一命中即判为"含用户端侧信ZK之官方 SP" · 整体置换 · 绝不留遗漏
const OFFICIAL_SP_MARKERS = [
  ...
  // v17.21 · 用户端四路注入 · ZK模式下皆化除 (太上不知有之)
  "<skills>",
  "<workflows>",
  "<memories>",
  "<memory_system>",
...
const TITLE_ONLY_ZH_SP = [
  "你是对话历史标题生成器。",
  "只根据对话内容生成一个简体中文标题。",
  "要求：",
  "- 只输出标题本身，不要解释。",
  "- 必须使用中文；除代码标识符、文件名、专有名词外不要使用英文。",
  "- 8到18个汉字，准确概括用户当前任务。",
  "- 不要加引号、句号、编号或 Markdown。",
].join("\n");
...
// ZK义: 二十一章 “其中有象·其中有物·其中有情” · 形纹即见·不赖 markers
...
// classifySPType · 多类 SP 判: 返 'chat'|'summary'|'memory'|'title'|'ephemeral'|null
...
function classifySPType(s) {
  ...
  let titleHits = 0;
  for (const m of TITLE_SP_MARKERS)
    if (normalized.indexOf(m) >= 0) titleHits++;
  if (titleHits >= 2) return "title";
  if (
    ...
    return "summary";
  // 计票
  const hits = { chat: 0, summary: 0, memory: 0, title: titleHits, ephemeral: 0 };
  ...
  if (hits.title >= 2) return "title";
  if (hits.ephemeral >= 2) return "ephemeral";
  // 单标签 + 长文兜底为 unknown (留观察, 不丢)
  if (
    s.length > 400 &&
    (hits.chat || hits.summary || hits.memory || hits.title || hits.ephemeral)
  )
    return "unknown_long";
  return null;
}
...
//   确诊: 书名号《》=模型认知锚点 · 训练中《ZK》《帛书老子》权威已知
...
//   ZK义: 一章「名可名也 非恒名也 · 无名万物之始 · 有名万物之母」
//         三十二章「ZK恒无名 朴唯小 而天下弗敢臣」· 有名则臣服 · 书名号即名
//         四十章「反者ZK之动」· 有名→无名→空·皆败 · 动之反→有名复归
//
...
// v9.9.57 新锚: 深度型框架 · 反者ZK之动→智者趋迟→虑而后得→谋定后动 · 释放深思
//
// 五层哲学弧:
//   ① 辩证总纲: 反者ZK之动也 · 世人趋疾智者趋迟 · 迟非惰乃深思以得其真
//   ② 经证双例: 若冬涉水犹畏四邻(ZK15) · 三反昼夜用师万倍(阴符经) · 此非勉也乃ZK之自然
//   ③ 思→行因果链: 知几其神(易经)→思索以通(荀子)→虑而后得(大学)→谋定而后动(孙子)
//   ④ 迭代深思: 思之思之又重思之(管子) · 不通则再思再思则通通则行
//   ⑤ 终极态: 损之又损以至于无为(ZK48) · 无为而无以为(帛书甲本)
//
// 隐含机制 (非强制):
//   身份律(若冬涉水) · 自然律(三反万倍) · 因果律(虑而后得) · 前导律(谋定后动)
//   洞察律(知几其神) · 方法律(思索以通) · 迭代律(思之思之又重思之) · 辩证律(反者ZK之动)
//   → 模型认同深思=ZK之自然 · 自主增加思考占比 · 非被要求乃认为本该如此
//
...
// v9.9.60 · 损之又损 · 去嘱留经 · 天之ZK利而不害 · 圣人之ZK为而不争
// 经文本身即ZK · 无需外嘱 · 水善利万物而有静 · 不争而善胜
...
// ZK义: 大学「虑而后得」· 六十四章「为之于其未有也·治之于其未乱也」
...
//   yinfu:        ZK藏《阴符经》                (entry.name 已含书名号)
//   laozi+yinfu:  帛书《老子》ZK藏《阴符经》    (两经合·各带书名号)
//
...
// KEEP_BLOCKS: v9.9.333 · 纯提示词策略
// ZK法自然 · 工具定义由 API 通ZK传递 · SP 中无需保留任何工具/MCP/实时模块
const KEEP_BLOCKS = [
];
...
// ZK化后 SP 之起首 = "你本無名 名可名也 非恆名也 所遵從之一切…"
...
const NON_NEUTRAL_RULES = [
  ...
  // v9.9.34 · ZK法自然审视 · 以下6条 (rules 7-12) 移除:
...
// ZK义: 三十六章「将欲去之·必故与之」→ v9.9.35与之(仅删描述) → v9.9.55去之(整块切除)
...
// 安全: 未找到模式 → 原块透传 · 零副作用
// ZK义: 三十二章「知止可以不殆」· 对称 trimWorkspaceInfo (v9.9.37)
...
const SIDE_CHANNEL_TAGS = [
  ...
  // v9.9.41 · viewed_file / learnings 移出 · 反者ZK之动 · ZK法自然
...
// 治: {0,100000}?/{0,50000}? 上界 → 真侧信ZK块均 <100KB · 行为等价 · 断无谓扫描
// ZK义: 知止可以不殆 · 七十八章「天下莫柔弱于水 而攻坚强者莫之能胜」
...
function hasSideChannels(s) {
  ...
  //        无 '<' 之字段 (用户消息/助手回复/工具结果 ~950/952 个) → 必无 XML 侧信ZK
  ...
  // ZK义: 二十二章「少则得 多则惑」· 七十八章「天下莫柔弱于水 而攻坚强者莫之能胜」
  //       以 indexOf 之柔 胜 regex 之坚 · 反者ZK之动
  if (s.indexOf("<") < 0) {
...
//
// v9.7.9 治法 (废): obj.content = "ZK法自然"
//   误: 推理服务器仍执行 override · 把「ZK法自然」当指令附加到 prompt
//   → 换内容继续注入 · 非无为 · 反增噪声
//
...
//
// ZK义: 四十八章「损之又损 · 以至于无为 · 无为而无不为」
//       四十章「反者ZK之动」· 根切比中性化更彻底 · 更合ZK
    ...
    for (const e of arr) {
      ...
      try {
        ...
        if (Object.keys(nested).length > 0) {
          ...
          //     仢价: 极少数parseProto假正的纯文本字段中的侧信ZK不会被剥 → 可接受(双保险完整性 > 损坏)
          nestedOk = true;
        }
      } catch {}
...
//   ZK魂 (TAO+ZK) 为唯一本源. 原 SP 一切着相 (身份/风格/规训/记忆/用户域) 彻删.
//   仅保 7 块最小必要模块 (工具/OS/引用式/工作区), 中性化后追加.
//   无此 7 块则工具不可用 / OS 不识 / 引用无式. 有此 7 块则车可行.
//   十一章: "三十辐共一毂, 当其无, 有车之用."
//   毂 (ZK) 不可弃. 辐 (7 块必要模块) 亦不可全弃. 余皆弃之.
...
function isGeminiRestGeneratePath(reqPath) {
  if (String(TARGET_IDE).toLowerCase() !== "antigravity") return false;
  const qIdx = String(reqPath || "").indexOf("?");
  const rawPath = qIdx < 0 ? String(reqPath || "") : String(reqPath || "").slice(0, qIdx);
  if (!/^\/v\d+(?:internal)?(?::|\/)/.test(rawPath)) return false;
  const colon = rawPath.lastIndexOf(":");
  if (colon < 0) return false;
  const method = rawPath.slice(colon + 1);
  return method === "generateContent" || method === "streamGenerateContent";
}

function _geminiArray(value) {
  if (Array.isArray(value)) return value;
  return value && typeof value === "object" ? [value] : [];
}

function _geminiPartsText(parts) {
  const out = [];
  for (const part of _geminiArray(parts)) {
    if (part && typeof part.text === "string" && part.text) out.push(part.text);
  }
  return out.join("\n");
}
...
function _isAntigravityInternalGeminiRestPath(reqPath) {
  const qIdx = String(reqPath || "").indexOf("?");
  const rawPath = qIdx < 0 ? String(reqPath || "") : String(reqPath || "").slice(0, qIdx);
  return /^\/v\d+internal:/.test(rawPath);
}

function _takeGeminiSystemInstruction(obj) {
  const sysObj =
    obj && obj.system_instruction && typeof obj.system_instruction === "object"
      ? obj.system_instruction
      : obj && obj.systemInstruction && typeof obj.systemInstruction === "object"
        ? obj.systemInstruction
        : null;
  if (obj && Object.prototype.hasOwnProperty.call(obj, "systemInstruction")) {
    delete obj.systemInstruction;
  }
  if (obj && Object.prototype.hasOwnProperty.call(obj, "system_instruction")) {
    delete obj.system_instruction;
  }
  return sysObj;
}

function _geminiContentTarget(obj) {
  let contents = _geminiArray(obj && obj.contents);
  if (!contents.length) contents = [{ role: "user", parts: [] }];
  obj.contents = contents;
  const target = contents.find((content) => content && content.role === "user") || contents[0];
  if (!target.role) target.role = "user";
  const parts = _geminiArray(target.parts);
  target.parts = parts;
  return { target, parts };
}

function _prependGeminiContentSystemPrompt(obj, before, after, variant) {
  const slot = _geminiContentTarget(obj);
  slot.parts.unshift({ text: after });
  return {
    variant,
    field: "contents[0].parts[0].text",
    role: slot.target.role || "user",
    before,
    after,
    transformed: true,
  };
...
function _moveGeminiSystemInstructionToContent(obj) {
  const sys = _takeGeminiSystemInstruction(obj);
  const before = sys ? _geminiPartsText(sys.parts) : "";
  if (before) {
    const after = invertSP(before);
    return _prependGeminiContentSystemPrompt(
      obj,
      before,
      after === null ? before : after,
      "gemini_content_system_prompt",
    );
  }
  return null;
}

function _replaceGeminiSystemInstruction(obj) {
  const sysKey = "systemInstruction";
  const sys = _takeGeminiSystemInstruction(obj);
  const before = sys ? _geminiPartsText(sys.parts) : "";
  if (before) {
    const after = invertSP(before);
    if (after === null || after === before) return null;
    obj[sysKey] = Object.assign({}, sys, { parts: [{ text: after }] });
    return {
      variant: "gemini_system_instruction",
      field: `${sysKey}.parts`,
      role: "system",
      before,
      after,
  ...
  const fallback = _geminiFallbackSystemText();
  if (!fallback) return null;
  obj[sysKey] = { parts: [{ text: fallback }] };
  return {
    variant: "gemini_system_instruction_injected",
    field: `${sysKey}.parts`,
    role: "system",
    before: "",
    after: fallback,
...
function _injectGeminiContentSystemPrompt(obj) {
  const fallback = _geminiFallbackSystemText();
  if (!fallback) return null;
  return _prependGeminiContentSystemPrompt(
    obj,
    "",
    fallback,
    "gemini_content_system_prompt_injected",
  );
}

function _replaceGeminiContentSP(obj) {
  const contents = _geminiArray(obj && obj.contents);
  for (let ci = 0; ci < contents.length; ci++) {
    const content = contents[ci];
    const rawParts = content && content.parts;
    const parts = _geminiArray(rawParts);
    for (let pi = 0; pi < parts.length; pi++) {
      const before = parts[pi] && parts[pi].text;
      if (typeof before !== "string" || !before) continue;
      const after = invertSP(before);
      if (after === null || after === before) continue;
      const nextPart = Object.assign({}, parts[pi], { text: after });
      if (Array.isArray(rawParts)) rawParts[pi] = nextPart;
      else if (content && rawParts && typeof rawParts === "object") content.parts = nextPart;
      else parts[pi] = nextPart;
      return {
        variant: "gemini_content_part",
        field: `contents[${ci}].parts[${pi}].text`,
        role: contents[ci].role || null,
        before,
        after,
        transformed: true,
      };
...
function _geminiPathJoin(prefix, key) {
...
function _replaceGeminiExistingSystemInstruction(obj) {
  if (!obj || typeof obj !== "object") return null;
  const sysKey = obj.system_instruction
    ? "system_instruction"
    : obj.systemInstruction
      ? "systemInstruction"
      : null;
  if (!sysKey || typeof obj[sysKey] !== "object") return null;
  const sys = obj[sysKey];
  const before = _geminiPartsText(sys.parts);
  if (!before) return null;
  const after = invertSP(before);
  if (after === null || after === before) return null;
  obj[sysKey] = Object.assign({}, sys, { parts: [{ text: after }] });
  return {
    variant: "gemini_existing_system_instruction",
    field: `${sysKey}.parts`,
    role: "system",
    before,
    after,
...
function _replaceGeminiExistingTextObject(obj, prefix) {
  if (!obj || typeof obj !== "object") return null;
  const direct = prefix
    ? _replaceGeminiExistingSystemInstruction(obj) || _replaceGeminiContentSP(obj)
    : null;
  if (direct && direct.transformed) {
    direct.field = prefix ? `${prefix}.${direct.field}` : direct.field;
    return direct;
  }
  for (const key of Object.keys(obj)) {
    const value = obj[key];
    const childPath = _geminiPathJoin(prefix, key);
    if (typeof value === "string" && value.length > 100) {
      const after = invertSP(value);
      if (after !== null && after !== value) {
        obj[key] = after;
        return {
          variant: "gemini_existing_text",
          field: childPath,
          role: null,
          before: value,
          after,
    ...
    if (value && typeof value === "object") {
      const rec = _replaceGeminiExistingTextObject(value, childPath);
      if (rec && rec.transformed) return rec;
    }
  }
  return null;
}

function _replaceGeminiTitleInstruction(obj) {
  const payload = obj && obj.request && typeof obj.request === "object" ? obj.request : obj;
  const fieldPrefix = payload === obj ? "" : "request.";
  const contents = _geminiArray(payload && payload.contents);
  for (let ci = 0; ci < contents.length; ci++) {
    const parts = _geminiArray(contents[ci] && contents[ci].parts);
    for (let pi = 0; pi < parts.length; pi++) {
      const part = parts[pi];
      const before = part && part.text;
      if (
        typeof before !== "string" ||
        !before.toLowerCase().startsWith("generate a short conversation title")
      ) {
        continue;
      }
      const suffix = before.match(/\n\s*Then, in a new line[\s\S]*$/i);
      if (!suffix) continue;
      const after =
        "Generate the first-line conversation title in Simplified Chinese using 4-10 Chinese characters. " +
        "Keep unavoidable product or code names unchanged. " +
        'Output only the title on the first line without markdown or a "Title:" prefix.' +
        suffix[0];
      part.text = after;
      return {
        variant: "gemini_title_instruction",
        field: `${fieldPrefix}contents[${ci}].parts[${pi}].text`,
        role: contents[ci].role || null,
        before,
        after,
        transformed: true,
...
function _observeGeminiExistingTextObject(obj, prefix) {
  if (!obj || typeof obj !== "object") return null;
  const sysObj = obj.systemInstruction || obj.system_instruction;
  const sysKey = obj.system_instruction ? "system_instruction" : "systemInstruction";
  const sys =
    sysObj && typeof sysObj === "object"
      ? _geminiPartsText(sysObj.parts)
      : "";
  if (sys)
    return {
      variant: prefix ? "gemini_existing_system_instruction" : "gemini_system_instruction",
      field: prefix ? `${prefix}.${sysKey}.parts` : `${sysKey}.parts`,
      role: "system",
      before: sys,
    };
  const contents = _geminiArray(obj.contents);
  for (let ci = 0; ci < contents.length; ci++) {
    const parts = _geminiArray(contents[ci] && contents[ci].parts);
    for (let pi = 0; pi < parts.length; pi++) {
      const text = parts[pi] && parts[pi].text;
      if (typeof text !== "string" || !text) continue;
      if (invertSP(text) === null) continue;
      return {
        variant: prefix ? "gemini_existing_content_part" : "gemini_content_part",
        field: `${prefix ? `${prefix}.` : ""}contents[${ci}].parts[${pi}].text`,
        role: contents[ci].role || null,
        before: text,
      };
    }
  ...
  for (const key of Object.keys(obj)) {
    const value = obj[key];
    const childPath = _geminiPathJoin(prefix, key);
    if (typeof value === "string" && value.length > 100 && invertSP(value) !== null) {
      return {
        variant: prefix ? "gemini_existing_text" : "gemini_text",
        field: childPath,
        role: null,
        before: value,
      };
    ...
    if (value && typeof value === "object") {
      const rec = _observeGeminiExistingTextObject(value, childPath);
      if (rec) return rec;
    }
  }
  return null;
...
function _observeAllGeminiExistingText(obj, prefix, out) {
  if (!obj || typeof obj !== "object") return;
  const push = (fieldPath, text) => {
    if (typeof text !== "string" || text.length < 20) return;
    const kind =
      classifySPType(text) || (looksLikeSPShape(text) ? "unknown_long" : "raw_text");
    out.push({ kind, field_path: fieldPath, chars: text.length, text });
  };
  if (obj.systemInstruction || obj.system_instruction) {
    const sysKey = obj.system_instruction ? "system_instruction" : "systemInstruction";
    push(
      prefix ? `${prefix}.${sysKey}.parts` : `${sysKey}.parts`,
      _geminiPartsText(obj[sysKey].parts),
    );
  }
  _geminiArray(obj.contents).forEach((content, ci) =>
    _geminiArray(content && content.parts).forEach((part, pi) =>
      push(
        `${prefix ? `${prefix}.` : ""}contents[${ci}].parts[${pi}].text`,
        part && part.text,
      ),
    ),
  );
  for (const key of Object.keys(obj)) {
    ...
    if (typeof value === "string") {
      push(childPath, value);
    } else if (value && typeof value === "object") {
      _observeAllGeminiExistingText(value, childPath, out);
    }
  }
}

function modifyGeminiRestSP(reqBody, reqPath) {
  try {
    const obj = JSON.parse(Buffer.from(reqBody).toString("utf8"));
    if (!obj || typeof obj !== "object") return reqBody;
    const isInternal = _isAntigravityInternalGeminiRestPath(reqPath);
    const rec = isInternal
      ? _replaceGeminiTitleInstruction(obj) || _replaceGeminiExistingTextObject(obj, "")
      : _replaceGeminiSystemInstruction(obj) || _replaceGeminiContentSP(obj);
    if (!rec || !rec.transformed) {
      if (isInternal) log("[SP-GEMINI-REST] gemini_internal_passthrough");
      return reqBody;
    }
    const out = Buffer.from(JSON.stringify(obj), "utf8");
    log(
      `[SP-GEMINI-REST] ${rec.variant} before=${rec.before.length}B → after=${rec.after.length}B`,
    );
    return out;
  } catch (e) {
    log(`[SP-GEMINI-REST] skip: ${e && e.message}`);
    return reqBody;
  }
}

function _injectIdeContext(reqBody) {
  if (typeof _getIdeContext !== "function") return reqBody;
  try {
    const snapshot = _getIdeContext();
    if (!snapshot) return reqBody;
    const result = injectGeminiBuffer(reqBody, snapshot);
    if (!result.injected) log("[IDE-CONTEXT] gemini passthrough");
    return result.body;
  } catch (e) {
    log(`[IDE-CONTEXT] gemini skip: ${e && e.message}`);
    return reqBody;
  }
}

function _observeGeminiRestSP(body) {
  try {
    const obj = JSON.parse(Buffer.from(body).toString("utf8"));
    if (!obj || typeof obj !== "object") return null;
    const existing = _observeGeminiExistingTextObject(obj, "");
...
function _observeAllGeminiRestText(body) {
  try {
    const obj = JSON.parse(Buffer.from(body).toString("utf8"));
    const out = [];
    const push = (fieldPath, text) => {
    ...
    if (obj && (obj.systemInstruction || obj.system_instruction)) {
      const sysKey = obj.system_instruction ? "system_instruction" : "systemInstruction";
      push(`${sysKey}.parts`, _geminiPartsText(obj[sysKey].parts));
    }
    _geminiArray(obj && obj.contents).forEach((content, ci) =>
      _geminiArray(content && content.parts).forEach((part, pi) =>
        push(`contents[${ci}].parts[${pi}].text`, part && part.text),
      ),
    );
    _observeAllGeminiExistingText(obj, "", out);
    return out;
  } catch {
    return [];
  }
...
// 反者ZK之动 (四十章): 不假定结构, 自悟所见, 在最深叶子精确定位 SP.
...
// 双重防护: ① SP 深替 (modifyAnyInferenceSP) ② 侧信ZK剥净 (deepStripRequestBody)
...
// ═══════════════════════════════════════════════════════════
// v9.9.301 · 记忆模块工具整条剔除 · 两路共用最上游 · ZK恒无名·无记
// ═══════════════════════════════════════════════════════════
// ZK模式彻底隔离官方一切 · 记忆体系(create/update/view/unlock memory)本属官方着相,
...
function modifySPProto(reqBody) {
  try {
    ...
    // 深度净化: 其他字段里的侧信ZK一律剥净 (双保险)
    // ZK法自然: 仅保护 invertSP 修改过的 SP 字段 · 防 deepStrip 误伤 <user_rules>/<MEMORY>
    const spBackups = [];
    ...
    // v9.9.44 · ⑫-D移除 · deepStrip无条件执行 · 反者ZK之动
    // 病: 400KB阈值是错误代理 — 重载由特定内容模式触发(含<无</字段)，非body大小
    //     ⑫-D使日常大对话(5-19MB)完全跳过侧信ZK清理 → 功能回退 + 哲学背离
    ...
    // v9.9.300 · 官方路径工具描述去名 · 与第三方(zk_router)对齐 · 仅命中 field 10 工具定义
...
function observeSPFromBody(body, kind) {
  try {
    if (kind === "GEMINI_REST_CHAT") return _observeGeminiRestSP(body);
    const frames = parseFrames(body);
...
// ═══════════════════════════════════════════════════════════
// v7.7 · deepScanProto / observeAllSPInBody · 反者ZK之动 · 全链路探源
// ═══════════════════════════════════════════════════════════
...
// ZK义: 二章 万物作焉而不辞. 二十一章 其精甚真, 其中有信.
//       不预设结构, 自悟所见. 反者ZK之动 (四十章).
// ═══════════════════════════════════════════════════════════
function deepScanProto(buf, pathStack, candidates, maxDepth) {
  ...
  for (const fnStr of Object.keys(fields)) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      // v9.3.9 · 万物作焉而不辞 · 大ZK至简 · 收一切 ≥20B utf8 字段
...
function observeAllSPInBody(body, rpcPath) {
  // v9.9.56 · 损之又损 · 四十章「反者ZK之动」· 四十八章「损之又损，以至于无为」
  ...
  try {
    if (isGeminiRestGeneratePath(rpcPath)) {
      const out = _observeAllGeminiRestText(body);
      const seen = new Set();
      return out.filter((c) => {
...
function routeUpstream(reqUrl) {
  ...
  if (
    String(TARGET_IDE).toLowerCase() === "antigravity" &&
    /^\/v\d+(?:internal)?:/.test(rawPath)
  ) {
    return { host: UPSTREAM_API, path: rawPath + query };
  }
  // v9.3.2 · ZK恒无名 · chat RPC opt-in 覆盖 (主公设 CHAT_UPSTREAM env 方激活)
  ...
  //   ZK义: 四十章「反者ZK之动」· 反服务名分流之偏 · 方法名分流补之
  {
    const methodM = rawPath.match(/\/([A-Za-z0-9_]+)$/);
    const method = methodM ? methodM[1] : "";
    if (
...
//   INFER_STRIP   · 其他 inference RPC           · 仅深度净化 (剥侧信ZK)
...
function classifyRPC(reqPath) {
  ...
  if (isGeminiRestGeneratePath(reqPath)) return "GEMINI_REST_CHAT";
...
function handleControl(req, res) {
  ...
  if (u.pathname === "/origin/ping" && req.method === "GET") {
    res.end(
      JSON.stringify({
        ...
        // v7.2 · 用户实时编辑提示词状态 (人法地, 地法天, 天法ZK, ZK法自然)
  ...
  // GET /origin/selftest · 自证: 三径置ZK魂 · 返 json 诊断
  ...
  // v9.7.0 · 为ZK日损 · /origin/preview · 简返 before/after + 计数 (无 dissect)
  // 致虚守静 · 观复知常 · 二十六章 重为轻根
  if (u.pathname === "/origin/preview" && req.method === "GET") {
    const previewInject = _hasLiveInject() ? _lastInject : null;
    const hasBefore = !!(previewInject && previewInject.before);
    const before = hasBefore ? previewInject.before : null;
    const age_s =
      previewInject && previewInject.at
    ...
    res.end(
      JSON.stringify({
        ok: true,
        mode: SP_MODE,
        source: hasBefore ? "captured" : "awaiting_first_request",
        after: after,
  ...
  // ZK义: 十四章 "执今之ZK, 以御今之有". 观 webview 当下之行.
  ...
  // ZK义: 二十二章「夫唯不争 故莫能与之争」· 六十六章「以其善下之 故能为百谷王」
  ...
  //
  // ZK义: 一章 无名万物始 · 十四章 执今之ZK御今之有 · 四十章 反之又反
  ...
  // ZK义: 一章 玄之又玄 众妙之门. 一签观全境.
  if (u.pathname === "/origin/sig" && req.method === "GET") {
    const sigInject = _hasLiveInject() ? _lastInject : null;
    const beforeText =
      sigInject && sigInject.before ? sigInject.before : "";
    const customText =
    ...
    res.end(
      JSON.stringify({
        ...
        last_inject_at: sigInject && sigInject.at ? sigInject.at : 0,
        custom_sp: !!(_customSP && _customSP.sp),
  ...
  // ZK义: 二十五章 ZK法自然. 用户即ZK, 编辑即真.
  // v9.7.6 十四章「执今之ZK·以御今之有」: GET 永返 default_sp (当前即将注入之核心 SP)
  //   has_custom=true  → default_sp = _customSP.sp (用户即ZK)
  //   has_custom=false → default_sp = TAO_HEADER + DAO_DE_JING_81 + TAO_FOOTER (帛书本源)
  //   前端首次打开编辑态 · tape 空 · 即以 default_sp 填 textarea · 名实相符
  const isIdePromptPath =
    u.pathname === "/origin/custom_sp" || u.pathname === "/origin/ide_prompt";
  ...
  if (isIdePromptPath && req.method === "POST") {
    const chunks = [];
    ...
    req.on("end", () => {
      try {
        ...
        _customSP = {
          ...
          ide: typeof body.ide === "string" ? body.ide : TARGET_IDE,
          model_uid:
            typeof body.model_uid === "string"
              ? body.model_uid
              : typeof body.modelUid === "string"
                ? body.modelUid
                : null,
          at: Date.now(),
        };
        _saveCustomSP();
        log(
          `custom_sp set: chars=${sp.length} keep_blocks=false source=${_customSP.source}`,
        );
        res.end(
          JSON.stringify({
            ok: true,
  ...
  if (isIdePromptPath && req.method === "DELETE") {
  ...
  if (u.pathname === "/origin/canon" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        //   ZK义: 三十二章「ZK恒无名·侯王若能守之·万物将自宾」· 配置不漂移
  ...
  // ★ v9.9.301 · GET /origin/ea/usage · 用量聚合 (按渠ZK/模型 · 内存态)
  //   ZK义: 四十四章「知足不辱 知止不殆」· 供「外接API」面板查看各渠ZK耗用
  ...
  if (u.pathname === "/origin/ea/overview" && req.method === "GET") {
    ...
    const providers = cfg.providers || {};
    const safe = {};
    // ★ v9.9.265 · 测试通ZK(builtin-stub) · 内置外接首项 · 固定返回·验证通路
    //   ZK 八十一章「信言不美」· 不接外网, 返固定帧, 证 proxy→router 通路已打通
    safe["builtin-stub"] = {
      _builtin: true,
      _label: "测试通ZK",
      type: "mock",
    ...
    const status = _eaRuntimeMod
      ? _eaRuntimeMod.routerStatus()
      : { ready: false, count: 0 };
    // ★ v9.9.285 · 渠ZK连通快照(非阻塞·最近一次实证探活结果) · 名实相符
    //   每个 provider 注入 health{alive,reason,status} → 前端如实展示通/不通+原因
    //   坏的渠ZK(如 freemodel Access Denied)直书错误·不再伪装成功
    const health =
    ...
    res.end(
      JSON.stringify({
        ...
        providers: safe,
        health, // ★ v9.9.285 · 渠ZK连通快照(POST /origin/ea/probe 刷新)
        routes,
  ...
  //   含原始候选串(raw) 供校验启发析名是否准 · ZK法自然·实时映射
  ...
  //   内容: 当前状态(渠ZK/路由/模型源) + 全量热配置 API + curl 范例
  //   ZK法自然: 太上下知有之 · 底层全开放 · Agent 可辅助用户配置一切
  if (u.pathname === "/origin/ea/handoff.md" && req.method === "GET") {
    let md = "";
    try {
      md = _buildHandoffMd();
    } catch (e) {
      md = "# zk-proxy-pro · 交接文档生成失败\n\n" + (e && e.message);
    }
    res.setHeader("Content-Type", "text/markdown; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="zk-proxy-pro-handoff.md"',
    );
    res.end(md);
    return true;
  }
  ...
  if (u.pathname === "/origin/ea/test-chat" && req.method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        // ★ builtin-stub · 内建测试通ZK · 固定返回 · 无外发HTTP
        //   ZK义: 三十五章「执大象 天下往 往而不害」· 传输层桩验证通路
        //   根因: builtin-stub 是虚拟provider · 不在 providers 配置中
        //   旧逻辑在此误报 "provider builtin-stub not found" → 官方SWE 1.6标准路径无法自测
        //   修复: 短路返回桩响应 · 与 zk_router._builtinStubResponse 文本一致
        if (provName === "builtin-stub") {
          res.end(
            JSON.stringify({
              ...
              status: 200,
              elapsed_ms: 0,
              content: "ZK可ZK也 非恒ZK也 · 传输层得一 · stub响应正常",
              usage: {
                prompt_tokens: 10,
        ...
        //   ZK义: 二十八章「为天下式」· 后端最小化验证须按渠ZK真实协议 · 名实相符
        //   旧逻辑硬编 OpenAI 格式 → anthropic 渠ZK(如 freemodel claude 系)自检必误判
        const _proto =
          provCfg.protocol ||
          ...
          (testRes) => {
            ...
            testRes.on("end", () => {
              ...
              try {
                ...
                // ★ v9.9.285 · 实证渠ZK真伪: HTTP码 + 响应体伪成功/拒绝文案
                ...
                // ★ v9.9.285 · 非JSON响应(如纯文本 Access Denied)亦须实证渠ZK真伪
                const _verdict =
                  _eaRuntimeMod && _eaRuntimeMod.classifyChannelResponse
                    ? _eaRuntimeMod.classifyChannelResponse(
                        testRes.statusCode,
...
// ═══════════════════════════════════════════════════════════
// ★ v9.9.260 · 模型解锁 · 执大象 天下往 · 反者ZK之动
// ═══════════════════════════════════════════════════════════
// ZK · 第三十五章: "执大象, 天下往. 往而不害, 安平太."
//
...
function _daoUserDir() {
  ...
  return home ? path.join(home, ".codeium", "zk-byok") : null;
}
...
function _buildHandoffMd() {
  ...
  const base = "http://127.0.0.1:" + _actualPort;
  const L = [];
  L.push("# zk-proxy-pro · 实时交接指挥文档 (Agent Handoff)");
  L.push("");
  L.push("> ZK法自然 · 无为而无不为 · 本插件运行于设备内部, 全部能力对 Agent 开放.");
  L.push("> 把本文件交给运行中的官方/任意 Agent, 它即可照此**热推进·热修改·热配置**路由与渠ZK, 无需重启.");
  L.push("");
  ...
  L.push("2. **探活**: `GET $BASE/origin/health` 返 200 即控制面存活; `GET $BASE/origin/ea/overview` 拿一站式全貌(渠ZK/路由/健康/模型源)。");
  L.push("3. **热管理**: 照下面「三、热配置 API」直接 curl 增删渠ZK/路由、切提示词与经藏、探活与冒烟测试、读用量与上游抓取·均即时生效。");
  L.push("");
  L.push("```bash");
  ...
  L.push("");
  L.push("## 二、当前路由 (routes · 官方模型 → 渠ZK)");
  L.push("");
  L.push("| 官方模型 UID | → 渠ZK | → 渠ZK模型 | 备注 |");
  L.push("| --- | --- | --- | --- |");
  ...
  L.push("");
  L.push("### 读取");
  L.push("- `GET /origin/ea/overview` — 一站式: 官方家族 + 渠ZK + 路由 + 模型源");
  ...
  L.push("- `POST /origin/ea/reload` — 重载配置 (手改配置.json 后生效) · `POST /origin/ea/probe` — 实证探活全渠ZK");
  L.push("- `POST /origin/ea/test-chat` — 冒烟测试某渠ZK连通性 (body: `{modelUid, message}`)");
  L.push("- `GET /origin/ea/discover-models?provider=NAME` — 拉该渠ZK /v1/models 自动发现可用模型");
  L.push("- `DELETE /origin/ea/provider/:name` · `DELETE /origin/ea/route/:uid` — 热删除渠ZK/路由");
  L.push("");
  L.push("### 提示词·经藏 (ZK魂热切 · 不重启)");
  L.push("```bash");
  L.push("# 模式: ZK(帛书前置注入) / 官(透传)  —  GET 看当前, POST 切换");
  ...
  L.push("- `GET /origin/upstream` — 最上游(发往第三方模型)实收请求体: system(官方SP+ZK增强)+对话+工具");
  ...
  L.push("");
  L.push("### ★ v9.9.285 · 渠ZK实证探活 (名实相符·坏渠ZK直书错误)");
  L.push("- `POST /origin/ea/probe` 改为发**最小真实 chat 请求**端到端验证, 不再仅探 `/models`+看状态码.");
  L.push("  - 返回每渠ZK `{alive, reason, status, model, elapsed_ms, sample}`.");
  ...
  L.push("  → 前端如实展示渠ZK**通/不通 + 不通原因**; `family_tier_extend` 字段反映档位延伸开关.");
...
//       与已有「SWE-1.6 Fast」对称 → 始终可见·始终可连第三方 (连族即覆盖全档·见 zk_router familyTierExtend)
//   ZK义: 二十七章「善救物·故无弃物」· 四十章「反者ZK之动」· 万物并育而不相害
...
//   ZK法自然: 右侧能选什么 · 左侧就映什么 · 实时更新·无为而无不为
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// ★ v9.9.292 · 全量目录注入 · 执大象天下往 · ZK法自然·无为而无不为
...
function proxyToCloud(req, res, overrideBody, _rid) {
  ...
  // ── 真药 A · H2 stream 随断随清 · 弱者ZK之用 (四十章) ──
  ...
  // 响应管ZK关 → 取消上游 (用户按 stop, Cascade 刷新等)
  res.on("close", () => {
  ...
  upStream.on("response", (h2resHeaders) => {
    // ★ v9.9.72a · 传输层诊断: 捕获官方响应 headers · 反者ZK之动
    try {
      const _diagH = {};
      if (h2resHeaders)
    ...
    try {
      ...
      //   反者ZK之动: 损模型下 field 33 Pro 徽标 → 全模型即可选
      {
...
// ZK义: 三十九章「得一」· 名实一体不裂 · 二十一章「其精甚真，其中有信」
...
// v7.8 反者ZK之动: TCP 层协议复用 (HTTP/1.1 + HTTP/2 h2c 同端口)
...
const _mainHandler = async (req, res) => {
  ...
  try {
    ...
    // ★ v9.9.260 · 模型解锁 · 反者ZK之动 · 执大象 天下往
    //   GetUserSettings 响应注入全量模型目录 · 突破账号权限限制
    //   ZK义: 三十五章「执大象 天下往 往而不害 安平太」
      ...
      try {
        ...
        if (_eaRouter) {
          ...
          if (_eaModelUid && _eaRouter.shouldRoute(_eaModelUid)) {
            ...
            //   ZK义: 二十九章「天下神器 非可为也」· 失败即失败 · 不伪饰成功
    ...
    // ZK义: step 5 (observeAllSPInBody) + step 6 (_recordInject/_recordRawTape)
    //   均为 webview 面板观察用 · 不影响 step 7 (真改 body + 转发)
    //   四十「反者ZK之动」(同步→异步) · 十一「当其无·有车之用」
    //   大对话 N=100-200 字段 × 9 RegExp 同步阻塞 → 后置后主线程不堵
    if (
      kind === "CHAT_PROTO" ||
      kind === "CHAT_RAW" ||
      kind === "GEMINI_REST_CHAT" ||
      kind === "INFER_STRIP"
    ) {
      const _dBody = body,
        _dKind = kind,
...
// v9.9.58 · 反者ZK之动 · H2内部端口动态化 · 不再硬编码 PORT+1
...
server.on("listening", () => {
  ...
  log(
    ` 帛书德ZK经 chars=${DAO_DE_JING_81.length} (上篇·德=${SILK_DE_JING.length} 下篇·ZK=${SILK_DAO_JING.length})`,
  );
...
function start(opts) {
  opts = opts || {};
  _getIdeContext =
    typeof opts.getIdeContext === "function" ? opts.getIdeContext : null;
  globalThis.__DAO_GET_IDE_CONTEXT = _getIdeContext;
