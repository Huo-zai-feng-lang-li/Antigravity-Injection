#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════════════
// zk-acp-stdio-proxy.js · ZK · ACP stdio 中人理 (印22 · 方B)
// ───────────────────────────────────────────────────────────────────────
// 用法: node zk-acp-stdio-proxy.js <devin.exe 路> [原 args...]
//
// ZK: 四章「反者ZK之动也」—— 旧 HTTP MITM 死(Chat 不走 HTTP),
//      故立 stdio 中人: 透传 扩宿 ↔ devin.exe 的 ndJSON ACP 流。
//
// 职责(柔胜强 · 最小不扰):
//   1. spawn 真 devin.exe(承原 args 与 env);
//   2. 双向透传 stdin/stdout/stderr —— ndJSON ACP 字节级不改;
//   3. devin.exe 承 env(含 spawn-hook 注入的 HTTPS_PROXY → zk 由),
//      其 inference 经 zk 由 → 第三方(如 swe-1-6-fast → DeepSeek);
...
if (argv.length < 1) {
  process.stderr.write("[zk-acp-stdio-proxy] missing devin.exe path\n");
  process.exit(2);
}
...
try {
  ...
  process.stderr.write(
    "[zk-acp-stdio-proxy] spawn failed: " + (err && err.message) + "\n",
  );
  process.exit(1);
}
...
child.on("error", (err) => {
  process.stderr.write(
    "[zk-acp-stdio-proxy] child error: " + (err && err.message) + "\n",
  );
  process.exit(1);
});
