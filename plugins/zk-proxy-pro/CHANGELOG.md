...
**变更**：
1. `_isModelUnlockEnabled()` 默认值从「启用」改为「禁用」，标记文件不存在时模型列表请求纯流式透传
2. 移除 proxyStart 后的 autoModelUnlock 自动调用，删除该死代码函数
3. GetUserSettings/GetUserStatus 从「缓冲-解析-合并-重序列化」变回流式直透，减少首字延迟
4. **保留**全部解锁代码与 `/origin/model_unlock` 手动端点：账号权限受限时可 POST `{enabled:true}` 手动恢复

**历史澄清**：v9.9.519 的 Failed to send 真实根因是模型改写被冲掉（当时在 old-compat-manager），与模型解锁无关——因为默认值本就是启用，删 autoModelUnlock 不改变解锁状态。

## v9.9.527 · 修复流式空闲超时误杀长时间终端任务
