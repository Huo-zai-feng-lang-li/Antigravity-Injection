...
function _allSettingsJsonPaths() {
  return IDE_SETTINGS_NAMES.map(_settingsJsonForIde)
    .filter(Boolean)
    .filter((sp) => {
      try {
...
function _settingsJsonPath() {
  for (const ide of TARGET_IDE_SETTINGS_NAMES) {
    const sp = _settingsJsonForIde(ide);
    if (sp) return sp;
  }
  return null;
}
...
async function clearAnchor() {
  ...
  // 方法2: 直写 settings.json · Antigravity 优先，兼容清理旧 IDE 残留
  let cleared = false;
...
<body data-port="${proxyPort}">
  ...
  <div id="editArea">
    <div class="edit-hint">自定义 · Ctrl+Enter 保存</div>
    <textarea id="editText" placeholder="输入自定义提示词"></textarea>
    <div class="edit-bar">
      <button class="eb save" id="editSave" title="\u4fdd\u5b58\u63d0\u793a\u8bcd (Ctrl+Enter)">\u2714 \u4fdd\u5b58</button>
...
(function() {
  ...
  function setModeUI(mode) {
    ...
    if (curMode === 'passthrough') $btnOff.classList.add('active');
    else $editToggle.classList.add('active');
  }
  $btnOff.addEventListener('click', function() {
    if (curMode === 'passthrough') return;
    if (editMode) {
      if (!isEditDirty()) _closeEditMode();
      else { $editStatus.textContent = '\u5df2\u4fee\u6539\uff0c\u8bf7\u5148\u4fdd\u5b58'; return; }
    }
  ...
  function updateEditCount() {
    ...
    if ($editSave) $editSave.disabled = !isEditDirty();
  }
  function openEditMode(focus) {
    setModeUI('invert');
    editMode = true;
  ...
  $editToggle.addEventListener('click', function() {
    if (editMode) {
      if (!isEditDirty()) _closeEditMode();
      else $editStatus.textContent = '\u5df2\u4fee\u6539\uff0c\u8bf7\u5148\u4fdd\u5b58';
      return;
    }
    openEditMode(true);
  });
  $editSave.addEventListener('click', function() {
    ...
    if (!isEditDirty()) { $editStatus.textContent = '\u672a\u4fee\u6539'; return; }
  ...
  _wdbg('boot-done', 'boot', { listeners: 'btnOff,editToggle,editSave,message[data,customSP,canonChanged]', ver: '9.9.31' });
})();
</script>
</body>
</html>`;
