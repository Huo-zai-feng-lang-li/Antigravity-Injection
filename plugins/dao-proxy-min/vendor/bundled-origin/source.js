...
const UPSTREAM_CHAT = process.env.CHAT_UPSTREAM || UPSTREAM_API;
...
//   v9.9.77 · 纯提示词策略 · 不再拼接任何官方工具/MCP/实时模块
// ═══════════════════════════════════════════════════════════
...
function classifySPType(s) {
  ...
  let titleHits = 0;
  for (const m of TITLE_SP_MARKERS) if (s.indexOf(m) >= 0) titleHits++;
...
function invertAnySP(spText) {
  try {
    ...
    if (t === "title") return TITLE_ONLY_ZH_SP;
    if (!_activeCanonText) return null;
...
function proxyToCloud(req, res, overrideBody) {
  ...
  log(
    `[official-req] ${req.method || "POST"} ${route.host}${route.path} <- ${req.url || ""}`,
  );
  ...
  try {
    ...
    if (SP_MODE === "invert") {
      if (kind === "CHAT_PROTO") {
        ...
        modified = modifyGeminiRestSP(body, req.url);
