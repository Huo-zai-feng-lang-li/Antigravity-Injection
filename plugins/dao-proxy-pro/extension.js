  ...
  try {
    ...
    if (!wasAnchored) {
      if (TARGET_IDE === "Antigravity") {
        ...
        vscode.window.showInformationMessage(
          `道Agent · 已启 :${port} · Antigravity 下次对话经反代生效`,
        );
        return;
      }
...
(function() {
  ...
  function openEditMode(focus, switchMode) {
    ...
    if (switchMode !== false) vsc.postMessage({ command: 'setMode', mode: 'dao' });
...
<body data-port="${proxyPort}">
  <!-- ── 三模块 Tab 栏 ── -->
  <div class="dao-tabs">
    <button class="dao-tab active" data-pane="paneEssence">① 提示词</button>
...
(function() {
  ...
  for (var ti = 0; ti < _tabs.length; ti++) {
    _tabs[ti].addEventListener('click', function() {
      ...
      var activePane = document.querySelector('.dao-pane.active');
