...
- **不需要为 3.7 及更低版本做特殊处理。** Gemini37 兼容模式是历史遗留，保留但不主动维护、不新增低版本适配逻辑。
- 新增高版本模型（3.9+）时，本插件动态映射自动适配（从请求 URL `/v1beta/models/{model}:generateContent` 提取实际模型名，改写 LS 占位符 `gemini-2.5-pro`），**不需要改任何代码**。
- 模型列表过滤/白名单由 old-compat-manager 负责，本插件不做模型过滤。
